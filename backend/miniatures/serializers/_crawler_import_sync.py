...
class CollectionSync:
    ...
    value or `None`. If no match is found, a new `Collection` is created with `source` and
    whichever of `external_id`/`name` were given.
    """

    def __init__(self, source, external_id=None, name=None):
        """Store the resolved `source` and the `external_id`/`name` to match/create by."""
        self.source = source
    ...
    def _create(self):
        """Create a new `Collection` using whichever of `external_id`/`name` was given."""
        return Collection.objects.create(
            source=self.source, external_id=self.external_id, name=self.name,
        )
