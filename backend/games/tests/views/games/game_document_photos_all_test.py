"""Tests for the game document photos/all.json view (DM/superuser only, includes hidden)."""
...
class TestGameDocumentPhotosAllView(TokenAuthRequestMixin):
    """Tests for GET /games/<slug>/documents/<document_id>/photos/all.json."""

    def setup_method(self):
        """Set up a game, a DM, an unrelated user, and a hidden document with a photo."""
    ...
    def _url(self, document_id=None, game_slug='test-game'):
        """Return the photos/all URL for the given document (defaults to fixture)."""
        document_id = document_id if document_id is not None else self.hidden_document.id
        return f'/games/{game_slug}/documents/{document_id}/photos/all.json'
