"""Tests for the game possession photo upload init endpoint."""
...
class TestGamePossessionPhotoUploadView(TokenAuthRequestMixin):
    """Tests for POST /games/<game_slug>/possessions/<possession_id>/photo_upload.json."""

    def setup_method(self):
        """Set up a game, a possession, a DM, a player, and an unrelated user."""
        self.game = GameFactory(name='Epic Quest', game_slug='epic-quest')
        self.possession = GamePossessionFactory(game=self.game, name='The Rusty Anchor')
    ...
    def _url(self, game_slug=None, possession_id=None):
        """Return the upload endpoint URL for the given game_slug/possession_id (defaults)."""
        ...
        return f'/games/{game_slug}/possessions/{possession_id}/photo_upload.json'

    def _post(self, client, payload, token=None, game_slug=None, possession_id=None):
        """Issue a POST request to the photo upload endpoint, optionally with a token."""
        return self.post(client, self._url(game_slug, possession_id), payload, token=token)
    ...
    def test_unknown_possession_id_returns_404(self, client):
        """Test that a non-existent possession_id returns 404."""
        response = self._post(
            client, {'filename': 'photo.jpg'}, token=self.dm_token, possession_id=99999
        )
        assert response.status_code == 404

    def test_possession_from_different_game_returns_404(self, client):
        """Test that a possession_id belonging to a different game returns 404."""
        other_game = GameFactory(name='Other Game', game_slug='other-game')
        other_possession = GamePossessionFactory(game=other_game, name='Other House')
        response = self._post(
            client, {'filename': 'photo.jpg'}, token=self.dm_token,
            possession_id=other_possession.id,
        )
        assert response.status_code == 404
    ...
    def test_happy_path_returns_201_with_upload_id_token_and_possession_id(self, client):
        """Test that a valid request from the DM returns 201 with the expected body."""
        response = self._post(client, {'filename': 'house.png'}, token=self.dm_token)
        assert response.status_code == 201
        data = json.loads(response.content)
        assert isinstance(data['upload_id'], int)
        assert data['token']
        assert data['possession_id'] == self.possession.id
    ...
    def test_happy_path_creates_upload_record(self, client):
        ...
        assert (
            upload.file_path
            == f'photos/games/epic-quest/possessions/{self.possession.id}/photo.png'
        )
    ...
    def test_happy_path_creates_game_possession_photo_record(self, client):
        """Test that the first upload creates a GamePossessionPhoto record with ready=False."""
        response = self._post(client, {'filename': 'house.png'}, token=self.dm_token)
        data = json.loads(response.content)
        upload = Upload.objects.get(pk=data['upload_id'])
        photo = GamePossessionPhoto.objects.get(path=upload.file_path)
        assert photo.game_possession == self.possession
        assert photo.ready is False
        assert GamePossessionPhoto.objects.filter(game_possession=self.possession).count() == 1

    def test_upload_and_photo_share_same_file_path(self, client):
        """Test that the Upload and GamePossessionPhoto records share the same file_path/path."""
        response = self._post(client, {'filename': 'house.jpg'}, token=self.dm_token)
        data = json.loads(response.content)
        upload = Upload.objects.get(pk=data['upload_id'])
        photo = GamePossessionPhoto.objects.get(game_possession=self.possession)
        assert upload.file_path == photo.path

    def test_superuser_can_upload(self, client):
        """Test that a superuser is allowed to upload a photo for any possession."""
        superuser = SuperUserFactory(username='admin', password='secret-password')
        token = Token.objects.create(user=superuser)
        response = self._post(client, {'filename': 'house.jpg'}, token=token)
        assert response.status_code == 201
    ...
    def test_player_of_game_returns_201(self, client):
        """Test that a player of the game can upload the possession's photo."""
        response = self._post(client, {'filename': 'house.jpg'}, token=self.player_token)
        assert response.status_code == 201
    ...
    def _attach_existing_photo(self):
        """Create and attach a GamePossessionPhoto to `self.possession`, simulating a finalize."""
        photo = GamePossessionPhoto.objects.create(
            game_possession=self.possession,
            path=f'photos/games/epic-quest/possessions/{self.possession.id}/photo.png',
            ready=True,
        )
    ...
    def test_reupload_reuses_existing_game_possession_photo_row(self, client):
        """Test that re-uploading reuses the same GamePossessionPhoto row, not a new one."""
        ...
        assert photo.path == f'photos/games/epic-quest/possessions/{self.possession.id}/photo.jpg'
        assert photo.ready is False

    def test_reupload_updates_game_possession_photo_path(self, client):
        """Test that re-uploading updates the reused photo's path to the new extension."""
        self._attach_existing_photo()

        response = self._post(client, {'filename': 'house.webp'}, token=self.dm_token)
        data = json.loads(response.content)
        upload = Upload.objects.get(pk=data['upload_id'])
        assert (
            upload.file_path
            == f'photos/games/epic-quest/possessions/{self.possession.id}/photo.webp'
        )
