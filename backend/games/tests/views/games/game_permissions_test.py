...
class TestGamePermissionsView(TokenAuthRequestMixin, TestCase):
    ...
    def test_non_existent_slug_returns_200_with_can_edit_false(self):
        """Test that a non-existent game slug returns 200 with can_edit/can_create_item False."""
        response = self.client.get('/games/no-such-game/permissions.json')
        assert response.status_code == 200
        data = json.loads(response.content)
        assert data == {'can_edit': False, 'can_create_item': False, 'can_create_document': False}

    def test_dm_can_edit(self):
        """Test that the game's DM gets can_edit/can_create_item True."""
        token = Token.objects.create(user=self.dm_user)
        response = self._get(self.client, token=token)
        assert response.status_code == 200
        data = json.loads(response.content)
        assert data == {'can_edit': True, 'can_create_item': True, 'can_create_document': True}

    def test_superuser_can_edit(self):
        """Test that a superuser gets can_edit/can_create_item True."""
    ...
    def test_non_dm_user_cannot_edit(self):
        """Test that a non-DM authenticated user gets can_edit/can_create_item False."""
    ...
    def test_staff_cannot_edit_but_can_create_item(self):
        """Test that a global Staff account gets can_edit False but can_create_item True."""
        ...
        response = self._get(self.client, token=token)
        data = json.loads(response.content)
        assert data == {'can_edit': False, 'can_create_item': True, 'can_create_document': True}

    def test_anonymous_cannot_edit(self):
        """Test that an unauthenticated request gets can_edit/can_create_item False."""
    ...
    def test_role_dm_can_edit_regardless_of_real_identity(self):
        """Test that ?role=dm grants can_edit/can_create_item True even for an anonymous caller."""
        response = self._get(self.client, query='role=dm')
        data = json.loads(response.content)
        assert data == {'can_edit': True, 'can_create_item': True, 'can_create_document': True}

    def test_role_superuser_can_edit(self):
        """Test that ?role=superuser grants can_edit/can_create_item True."""
        response = self._get(self.client, query='role=superuser')
        data = json.loads(response.content)
        assert data == {'can_edit': True, 'can_create_item': True, 'can_create_document': True}

    def test_role_staff_cannot_edit_but_can_create_item(self):
        """Test that ?role=staff grants can_create_item True but not can_edit."""
        response = self._get(self.client, query='role=staff')
        data = json.loads(response.content)
        assert data == {'can_edit': False, 'can_create_item': True, 'can_create_document': True}

    def test_role_player_alone_cannot_edit(self):
        """Test that ?role=player alone does not grant can_edit or can_create_item."""
        response = self._get(self.client, query='role=player')
        data = json.loads(response.content)
        assert data == {'can_edit': False, 'can_create_item': False, 'can_create_document': False}

    def test_role_dm_overrides_authenticated_non_dm_real_identity(self):
        """Test that ?role=dm grants can_edit/can_create_item True even when real caller isn't."""
        other = UserFactory(username='other', password='secret-password')
        token = Token.objects.create(user=other)
