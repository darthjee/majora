"""Tests for the game document files/all.json view (DM/superuser only, includes hidden)."""
...
class TestGameDocumentFilesAllView(TokenAuthRequestMixin):
    """Tests for GET /games/<slug>/documents/<document_id>/files/all.json."""

    def setup_method(self):
        """Set up a game, a DM, an unrelated user, and a hidden document with a file."""
    ...
    def _url(self, document_id=None, game_slug='test-game'):
        """Return the files/all URL for the given document (defaults to fixture)."""
        document_id = document_id if document_id is not None else self.hidden_document.id
        return f'/games/{game_slug}/documents/{document_id}/files/all.json'
    ...
    def test_dm_gets_200_for_hidden_document_files(self, client):
        """Test that a DM gets 200 with files from a hidden document."""
