"""Tests for the game documents/all.json view (DM/superuser only, includes hidden)."""
...
class TestGameDocumentsAllView(TokenAuthRequestMixin):
    """Tests for GET /games/<slug>/documents/all.json."""

    def setup_method(self):
        """Set up a game, a DM, an unrelated user, and visible/hidden documents."""
        ...
        self.visible_document = GameDocumentFactory(game=self.game, name='Visible Letter')
        self.hidden_document = GameDocumentFactory(
            game=self.game, name='Secret Letter', hidden=True,
        )

    def _url(self, game_slug='test-game'):
        """Return the documents/all URL for the given game slug (defaults to the fixture)."""
        return f'/games/{game_slug}/documents/all.json'
    ...
    def test_dm_gets_200_with_both_visible_and_hidden_documents(self, client):
        ...
        assert 'Visible Letter' in names
        assert 'Secret Letter' in names
    ...
    def test_response_includes_hidden_field_per_document(self, client):
        ...
        assert by_name['Visible Letter'] is False
        assert by_name['Secret Letter'] is True
    ...
    def test_url_by_name(self, client):
        ...
        url = reverse('game-documents-all', kwargs={'game_slug': 'test-game'})
