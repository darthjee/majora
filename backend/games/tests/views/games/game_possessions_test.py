"""Tests for the game possessions view."""
...
class TestGamePossessionsView(TestCase):
    """Tests for the GET /games/<slug>/possessions.json endpoint."""
    ...
    def test_returns_empty_list_when_no_possessions(self):
        """Test that an empty list is returned when the game has no possessions."""
        response = self.client.get('/games/test-game/possessions.json')
        assert response.status_code == 200
        assert json.loads(response.content) == []

    def test_returns_only_game_possessions(self):
        """Test that only possessions belonging to the game are returned."""
        GamePossessionFactory(game=self.game, name='The Rusty Anchor')
        GamePossessionFactory(game=self.other_game, name='Silver Manor')
        response = self.client.get('/games/test-game/possessions.json')
        data = json.loads(response.content)
        assert len(data) == 1
        assert data[0]['name'] == 'The Rusty Anchor'
    ...
    def test_returns_id_name_photo_path_fields(self):
        ...
        response = self.client.get('/games/test-game/possessions.json')
        data = json.loads(response.content)
        assert data[0]['id'] == possession.id
        assert data[0]['name'] == 'The Rusty Anchor'
    ...
    def test_excludes_hidden_possessions(self):
        """Test that a hidden game possession is excluded from the response."""
        GamePossessionFactory(game=self.game, name='Hidden Vault', hidden=True)
        response = self.client.get('/games/test-game/possessions.json')
        data = json.loads(response.content)
        assert data == []

    def test_includes_visible_possessions_alongside_hidden_ones(self):
        """Test that non-hidden possessions are still returned when hidden ones exist too."""
        visible = GamePossessionFactory(game=self.game, name='Visible House')
        GamePossessionFactory(game=self.game, name='Hidden Vault', hidden=True)
        response = self.client.get('/games/test-game/possessions.json')
        data = json.loads(response.content)
        assert len(data) == 1
    ...
    def test_does_not_include_hidden_field(self):
        ...
        GamePossessionFactory(game=self.game, name='House')
        response = self.client.get('/games/test-game/possessions.json')
        data = json.loads(response.content)
    ...
    def test_returns_404_for_unknown_game_slug(self):
        ...
        response = self.client.get('/games/unknown-game/possessions.json')
    ...
    def test_response_includes_per_page_header(self):
        ...
        response = self.client.get('/games/test-game/possessions.json?per_page=5')
        assert response['per_page'] == '5'

    def test_respects_page_param(self):
        """Test that ?page=N returns the correct page of results."""
        for i in range(5):
            GamePossessionFactory(game=self.game, name=f'Possession {i}')
        response = self.client.get('/games/test-game/possessions.json?page=2&per_page=3')
        assert response.status_code == 200
        data = json.loads(response.content)
        assert len(data) == 2
    ...
    def test_url_by_name(self):
        ...
        url = reverse('game-possessions', kwargs={'game_slug': 'test-game'})
        response = self.client.get(url)
        assert response.status_code == 200

    def test_returns_possessions_ordered_by_id(self):
        """Test that possessions are returned ordered by id."""
        first = GamePossessionFactory(game=self.game, name='First Possession')
        second = GamePossessionFactory(game=self.game, name='Second Possession')
        response = self.client.get('/games/test-game/possessions.json')
        data = json.loads(response.content)
        assert [item['id'] for item in data] == [first.id, second.id]


class TestGamePossessionsCreate(TokenAuthRequestMixin, TestCase):
    """Tests for POST /games/<slug>/possessions.json (issue #1074)."""
    ...
    def _url(self, game_slug='test-game'):
        """Return the possessions list/create URL for the given game slug."""
        return f'/games/{game_slug}/possessions.json'

    def _post(self, client, payload, token=None):
        """Issue a POST request to create a game possession, optionally with a token."""
        return self.post(client, self._url(), payload, token=token)

    def test_dm_can_create_possession(self):
        """Test that the game's DM can create a bare GamePossession."""
        token = Token.objects.create(user=self.dm_user)
        response = self._post(
            self.client,
            {'name': 'The Rusty Anchor', 'description': 'A cozy tavern.', 'hidden': True},
            token=token,
        )
    ...
    def test_create_persists_game_possession(self):
        """Test that the create endpoint persists a GamePossession."""
        token = Token.objects.create(user=self.dm_user)
        response = self._post(
            self.client,
            {'name': 'The Rusty Anchor', 'description': 'A cozy tavern.'},
            token=token,
        )
        data = json.loads(response.content)
        possession = GamePossession.objects.get(id=data['id'])
    ...
    def test_create_defaults_description_and_hidden(self):
        ...
        token = Token.objects.create(user=self.dm_user)
        response = self._post(self.client, {'name': 'The Rusty Anchor'}, token=token)
        data = json.loads(response.content)
        assert data['description'] == ''
        assert data['hidden'] is False

    def test_superuser_can_create_possession(self):
        """Test that a superuser can create a bare GamePossession."""
        superuser = SuperUserFactory(username='admin', password='secret-password')
        token = Token.objects.create(user=superuser)
        response = self._post(self.client, {'name': 'The Rusty Anchor'}, token=token)
        assert response.status_code == 201

    def test_staff_can_create_possession(self):
        """Test that a global Staff account can create a bare GamePossession."""
    ...
    def test_unauthenticated_returns_401(self):
        ...
        response = self._post(self.client, {'name': 'The Rusty Anchor'})
        assert response.status_code == 401

    def test_regular_player_can_create_possession(self):
        """Test that a regular player of the game can create a bare GamePossession."""
