...
class TestTreasureAccessView(TokenAuthRequestMixin, TestCase):
    ...
    def test_regular_user_returns_correct_user_context_fields(self):
    ...
    def test_non_existent_treasure_returns_200_without_can_edit(self):
        """Test that a non-existent treasure id returns 200 without a can_edit field."""
        response = self.get(
            self.client, '/treasures/999999/access.json', token=self.superuser_token
        )
        assert response.status_code == 200
        data = json.loads(response.content)
        assert 'can_edit' not in data
    ...
    def test_superuser_via_session_returns_is_superuser_true(self):
        ...
        session.save()
        response = self.client.get(f'/treasures/{self.treasure.id}/access.json')
        assert response.status_code == 200
        data = json.loads(response.content)
        assert data['is_superuser'] is True
