"""Tests for the NPC faction acquire (enlist) endpoint."""
...
class TestGameNpcFactionAcquireView(TokenAuthRequestMixin):
    """Tests for POST /games/<slug>/npcs/<id>/factions/acquire.json."""

    def setup_method(self):
        """Set up a game, an NPC, a DM, an unrelated user, and an available game faction."""
    ...
    def _url(self, character_id=None, game_slug=None):
        ...
        return f'/games/{game_slug}/npcs/{character_id}/factions/acquire.json'
    ...
    def test_dm_can_acquire_faction(self, client):
        """Test that a DM of the game can enlist an NPC into a faction."""
        response = self._post(
    ...
    def test_acquire_creates_character_faction_row(self, client):
        """Test that a CharacterFaction row is created linking the NPC to the faction."""
    ...
    def test_staff_can_acquire_faction(self, client):
        """Test that a global Staff user, not a DM, can enlist an NPC into a faction."""
    ...
    def test_plain_player_can_acquire_faction(self, client):
        """Test that a plain player of the game (not staff/dm) can enlist an NPC."""
        response = self._post(
    ...
    def test_unknown_game_faction_id_returns_404(self, client):
        ...
        response = self._post(client, {'game_faction_id': 99999}, token=self.dm_token)
        assert response.status_code == 404
...
@pytest.mark.django_db
class TestGameNpcFactionAcquireHidden(TokenAuthRequestMixin):
    """Tests for enlisting a hidden NPC into a faction."""

    def setup_method(self):
        """Set up a game, a hidden NPC, a DM, and an available game faction."""
        self.game = GameFactory(name='Test Game', game_slug='test-game')
        self.dm_user = UserFactory(username='dm_user', password='secret-password')
        PlayerFactory(game=self.game, user=self.dm_user, is_dm=True)
        self.dm_token = Token.objects.create(user=self.dm_user)
        ...
        self.game_faction = GameFactionFactory(game=self.game, name='Secret Cabal')

    def _post(self, client, token=None):
        """Issue a POST request to acquire a faction for the hidden NPC."""
        return self.post(
            client,
            f'/games/test-game/npcs/{self.hidden_npc.id}/factions/acquire.json',
            {'game_faction_id': self.game_faction.id},
            token=token,
        )

    def test_dm_can_acquire_for_hidden_npc(self, client):
        """Test that a DM can enlist a hidden NPC into a faction."""
        response = self._post(client, token=self.dm_token)
        assert response.status_code == 201
