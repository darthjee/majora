...
class TestGameNpcFullUpdateView(TokenAuthRequestMixin):
    ...
    def test_patch_allegiance_with_unrelated_user_returns_403(self, client):
        """Test that a non-editor cannot set private_allegiance/public_allegiance via PATCH."""
        other_user = UserFactory(username='other', password='secret-password')
        token = Token.objects.create(user=other_user)

        response = self._patch(
            client, {'private_allegiance': 'enemy', 'public_allegiance': 'ally'}, token=token
        )

        assert response.status_code == 403
        self.character.refresh_from_db()
        assert self.character.private_allegiance == 'neutral'
    ...
    def test_patch_allegiance_by_dm_is_persisted(self, client):
        ...
        assert response.status_code == 200
        self.character.refresh_from_db()
        assert self.character.private_allegiance == 'enemy'
        assert self.character.public_allegiance == 'ally'

    def test_patch_private_slain_only_is_persisted_and_leaves_other_fields_untouched(
        self, client,
    ):
        """Test that PATCH with only {"private_slain": true} persists it, other fields alone."""
        token = self._editor_token()

        response = self._patch(client, {'private_slain': True}, token=token)

        assert response.status_code == 200
        self.character.refresh_from_db()
        assert self.character.private_slain is True
        assert self.character.name == 'Gandalf'
