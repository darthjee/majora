"""Tests for the NPC money-only update (PUT) endpoint."""
...
class TestGameNpcMoneyView(TokenAuthRequestMixin):
    """Tests for the NPC money-only update endpoint."""

    def setup_method(self):
        """Set up a game, a DM, a regular player, and the NPC."""
        self.game = GameFactory(name='Test Game', game_slug='test-game')
        self.dm_user = UserFactory(username='dm_user', password='secret-password')
        PlayerFactory(game=self.game, user=self.dm_user, is_dm=True)
        self.character = CharacterFactory(
            name='Gandalf', game=self.game, player=None, money=100, npc=True,
        )
    ...
    def _url(self, character_id=None):
        ...
        return f'/games/test-game/npcs/{character_id}/money.json'
    ...
    def test_put_with_regular_player_of_the_game_returns_403(self, client):
        """Test that a regular player of the game cannot edit NPC money (key restriction)."""
    ...
    def test_put_returns_404_for_pc_id(self, client):
        """Test that 404 is returned when the id belongs to a PC."""
        pc = CharacterFactory(name='Frodo', game=self.game, npc=False)
        token = self._dm_token()

        response = self.put(client, self._url(character_id=pc.id), {'money': 200}, token=token)
