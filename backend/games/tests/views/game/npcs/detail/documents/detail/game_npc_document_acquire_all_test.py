"""Tests for the DM-only NPC document acquire-all endpoint (accepts hidden game documents)."""
...
class TestGameNpcDocumentAcquireAllView(TokenAuthRequestMixin):
    """Tests for POST /games/<slug>/npcs/<id>/documents/acquire/all.json."""

    def setup_method(self):
        """Set up a game, a DM, an NPC, an unrelated user, and a hidden game document."""
    ...
    def _url(self, character_id=None, game_slug=None):
        ...
        return f'/games/{game_slug}/npcs/{character_id}/documents/acquire/all.json'
    ...
    def _post(self, client, token=None, character_id=None, game_slug=None):
        ...
        return self.post(
            ...
            {'game_document_id': self.hidden_document.id}, token=token,
        )

    def test_dm_can_acquire_a_hidden_document_on_behalf_of_an_npc(self, client):
        """Test that a DM can acquire a hidden document on behalf of an NPC via the all-variant."""
        response = self._post(client, token=self.dm_token)
        assert response.status_code == 201
        data = json.loads(response.content)
        assert data['hidden'] is True

    def test_superuser_can_acquire_a_hidden_document(self, client):
        """Test that a superuser can acquire a hidden document via the all-variant."""
    ...
    def test_acquire_creates_character_document_row(self, client):
        """Test that acquiring a hidden document still creates the CharacterDocument row."""
        self._post(client, token=self.dm_token)
        assert CharacterDocument.objects.filter(
            character=self.character, game_document=self.hidden_document,
        ).exists()
    ...
    def test_url_by_name(self, client):
        ...
        url = reverse(
            'game-npc-document-acquire-all',
            kwargs={'game_slug': self.game.game_slug, 'character_id': self.character.id},
        )
        response = client.post(
            url,
            data=json.dumps({'game_document_id': self.hidden_document.id}),
