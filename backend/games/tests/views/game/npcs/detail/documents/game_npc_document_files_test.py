"""Tests for the NPC document files-list view."""
...
class TestGameNpcDocumentFilesView(TokenAuthRequestMixin):
    """Tests for GET /games/<slug>/npcs/<id>/documents/<document_id>/files.json."""
    ...
    def _url(self, document_id=None, character_id=None, game_slug='test-game'):
        ...
        return f'/games/{game_slug}/npcs/{character_id}/documents/{document_id}/files.json'
...
@pytest.mark.django_db
class TestGameNpcDocumentFilesHidden(TokenAuthRequestMixin):
    """Tests for the hidden-NPC visibility gate in game_npc_document_files."""

    def setup_method(self):
        """Set up a hidden NPC holding a document with a file."""
        self.game = GameFactory(name='Test Game', game_slug='test-game')
    ...
    def _url(self):
        """Return the files list URL for the hidden NPC's held document."""
        return (
            f'/games/test-game/npcs/{self.hidden_npc.id}'
            f'/documents/{self.character_document.id}/files.json'
        )

    def test_hidden_npc_document_files_returns_404_for_anonymous(self, client):
        """Test that an anonymous request to a hidden NPC's document files gets 404."""
        response = self.get(client, self._url())
        assert response.status_code == 404

    def test_hidden_npc_document_files_returns_200_for_dm(self, client):
        """Test that a DM can access a hidden NPC's document files."""
    ...
    def test_hidden_npc_document_files_returns_200_for_superuser(self, client):
        """Test that a superuser can access a hidden NPC's document files."""
        superuser = SuperUserFactory(username='admin', password='secret-password')
        token = Token.objects.create(user=superuser)
        response = self.get(client, self._url(), token=token)
        assert response.status_code == 200

    def test_hidden_npc_document_files_404_response_includes_x_skip_cache_header(self, client):
        """Test that a 404 response for a hidden NPC's document files includes X-Skip-Cache."""
        response = self.get(client, self._url())
