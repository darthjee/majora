"""Tests for the PC items/all.json view (dm, owner, or admin only; includes hidden)."""
...
class TestGamePcItemsAllView(TokenAuthRequestMixin):
    """Tests for GET /games/<slug>/pcs/<id>/items/all.json."""
    ...
    def setup_method(self):
        ...
        self.hidden_item = GameItemFactory(game=self.game, name='Hidden Gem')
        CharacterItem.objects.create(character=self.character, game_item=self.visible_item)
        CharacterItem.objects.create(
            character=self.character, game_item=self.hidden_item, hidden=True,
        )

    def _url(self, character_id=None, game_slug='test-game'):
        """Return the items/all URL for the given character (defaults to the fixture)."""
        character_id = character_id if character_id is not None else self.character.id
        return f'/games/{game_slug}/pcs/{character_id}/items/all.json'
    ...
    def test_owner_gets_200_with_both_visible_and_hidden_items(self, client):
        """Test that the PC's owning player gets 200 with both visible and hidden items."""
    ...
    def test_dm_gets_200_with_both_visible_and_hidden_items(self, client):
        """Test that the game's DM gets 200 with both visible and hidden items."""
