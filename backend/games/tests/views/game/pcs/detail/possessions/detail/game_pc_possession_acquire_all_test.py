"""Tests for the DM-only PC possession acquire-all endpoint (accepts hidden game possessions)."""
...
class TestGamePcPossessionAcquireAllView(TokenAuthRequestMixin):
    """Tests for POST /games/<slug>/pcs/<id>/possessions/acquire/all.json."""

    def setup_method(self):
        """Set up a game, a DM, a PC (with an owning player), and a hidden game possession."""
    ...
    def _url(self, character_id=None, game_slug=None):
        ...
        return f'/games/{game_slug}/pcs/{character_id}/possessions/acquire/all.json'
    ...
    def _post(self, client, token=None, character_id=None, game_slug=None):
        ...
        return self.post(
            ...
            {'game_possession_id': self.hidden_possession.id}, token=token,
        )

    def test_dm_can_acquire_a_hidden_possession_on_behalf_of_a_pc(self, client):
        """Test that a DM can acquire a hidden possession on behalf of a PC via the all-variant."""
        response = self._post(client, token=self.dm_token)
        assert response.status_code == 201
        data = json.loads(response.content)
        assert data['hidden'] is True

    def test_superuser_can_acquire_a_hidden_possession(self, client):
        """Test that a superuser can acquire a hidden possession via the all-variant."""
    ...
    def test_acquire_creates_character_possession_row(self, client):
        """Test that acquiring a hidden possession still creates the CharacterPossession row."""
        self._post(client, token=self.dm_token)
        assert CharacterPossession.objects.filter(
            character=self.character, game_possession=self.hidden_possession,
        ).exists()
    ...
    def test_url_by_name(self, client):
        ...
        url = reverse(
            'game-pc-possession-acquire-all',
            kwargs={'game_slug': self.game.game_slug, 'character_id': self.character.id},
        )
        response = client.post(
            url,
            data=json.dumps({'game_possession_id': self.hidden_possession.id}),
