"""Tests for the PC possessions/all.json view (dm, owner, or admin only; includes hidden)."""
...
class TestGamePcPossessionsAllView(TokenAuthRequestMixin):
    """Tests for GET /games/<slug>/pcs/<id>/possessions/all.json."""
    ...
    def setup_method(self):
        ...
        self.visible_possession = GamePossessionFactory(game=self.game, name='Visible Vault')
        self.hidden_possession = GamePossessionFactory(game=self.game, name='Hidden Vault')
        CharacterPossession.objects.create(
            character=self.character, game_possession=self.visible_possession,
        )
        CharacterPossession.objects.create(
            character=self.character, game_possession=self.hidden_possession, hidden=True,
        )

    def _url(self, character_id=None, game_slug='test-game'):
        """Return the possessions/all URL for the given character (defaults to the fixture)."""
        character_id = character_id if character_id is not None else self.character.id
        return f'/games/{game_slug}/pcs/{character_id}/possessions/all.json'
    ...
    def test_owner_gets_200_with_both_visible_and_hidden_possessions(self, client):
        """Test that the PC's owning player gets 200 with both visible and hidden possessions."""
        response = self.get(client, self._url(), token=self.owner_token)
        assert response.status_code == 200
        data = json.loads(response.content)
        names = [item['name'] for item in data]
        assert 'Visible Vault' in names
        assert 'Hidden Vault' in names

    def test_dm_gets_200_with_both_visible_and_hidden_possessions(self, client):
        """Test that the game's DM gets 200 with both visible and hidden possessions."""
    ...
    def test_response_includes_hidden_field_per_possession(self, client):
        ...
        assert by_name['Visible Vault'] is False
