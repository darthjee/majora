"""Tests for the restricted PC possession remove-all endpoint (dm/admin/owner; incl. hidden)."""
...
class TestGamePcPossessionRemoveAllView(TokenAuthRequestMixin):
    """Tests for POST /games/<slug>/pcs/<id>/possessions/remove/all.json."""

    def setup_method(self):
        """Set up a game, a DM, a PC with an owning player, and a hidden owned possession."""
        ...
        self.game_possession = GamePossessionFactory(game=self.game, name='Secret Vault')
    ...
    def _url(self, character_id=None, game_slug=None):
        ...
        return f'/games/{game_slug}/pcs/{character_id}/possessions/remove/all.json'
    ...
    def _post(self, client, token=None, character_id=None, game_slug=None):
        ...
        return self.post(
            ...
            {'game_possession_id': self.game_possession.id}, token=token,
        )

    def test_dm_can_remove_a_hidden_possession(self, client):
        """Test that a DM can remove a hidden possession on behalf of a PC via the all-variant."""
    ...
    def test_superuser_can_remove_a_hidden_possession(self, client):
        """Test that a superuser can remove a hidden possession via the all-variant."""
        superuser = SuperUserFactory(username='admin', password='secret-password')
        token = Token.objects.create(user=superuser)
        response = self._post(client, token=token)
        assert response.status_code == 204

    def test_owning_player_can_remove_a_hidden_possession(self, client):
        """Test that a PC's own owning player can remove a hidden possession via the all-variant."""
