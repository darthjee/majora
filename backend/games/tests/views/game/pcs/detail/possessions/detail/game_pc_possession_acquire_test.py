"""Tests for the PC possession acquire endpoint."""
...
class TestGamePcPossessionAcquireView(TokenAuthRequestMixin):
    """Tests for POST /games/<slug>/pcs/<id>/possessions/acquire.json."""
    ...
    def _url(self, character_id=None, game_slug=None):
        ...
        return f'/games/{game_slug}/pcs/{character_id}/possessions/acquire.json'
    ...
    def test_editor_can_acquire_possession(self, client):
        """Test that an authorized editor can acquire an existing game possession."""
        response = self._post(
    ...
    def test_acquire_creates_character_possession_row(self, client):
        """Test that a CharacterPossession row links the character to the acquired possession."""
        self._post(
            client, {'game_possession_id': self.game_possession.id}, token=self._editor_token(),
        )
    ...
    def test_acquire_response_includes_expected_fields(self, client):
        """Test that the response exposes the acquired CharacterPossession's detail fields."""
        response = self._post(
            client, {'game_possession_id': self.game_possession.id}, token=self._editor_token(),
        )
        ...
        assert data['description'] == self.game_possession.description
        assert data['hidden'] is False

    def test_hidden_defaults_to_game_possession_hidden_when_omitted(self, client):
        """Test that hidden defaults to the (non-hidden) GamePossession's own hidden value."""
        response = self._post(
            client, {'game_possession_id': self.game_possession.id}, token=self._editor_token(),
        )
        data = json.loads(response.content)
    ...
    def test_hidden_respects_submitted_value(self, client):
        """Test that an explicit hidden value overrides the GamePossession's own default."""
        response = self._post(
            client, {'game_possession_id': self.game_possession.id, 'hidden': True},
    ...
    def test_game_possession_not_in_game_returns_404(self, client):
        """Test that a game possession not available in this game returns 404."""
        other_game = GameFactory(name='Other Game', game_slug='other-game')
        other_possession = GamePossessionFactory(game=other_game, name='Orb Vault')
        response = self._post(
    ...
    def test_dm_can_acquire_possession(self, client):
        """Test that a DM of the game can acquire a possession on behalf of a PC."""
        response = self._post(
    ...
    def test_staff_can_acquire_possession(self, client):
        """Test that a global Staff user, neither owner nor DM, can acquire a possession."""
    ...
    def test_unknown_game_possession_id_returns_404(self, client):
        ...
        response = self._post(client, {'game_possession_id': 99999}, token=self._editor_token())
        assert response.status_code == 404
