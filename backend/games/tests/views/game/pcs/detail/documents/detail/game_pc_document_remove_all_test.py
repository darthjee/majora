"""Tests for the restricted PC document remove-all endpoint (dm/admin/owner; accepts hidden)."""
...
class TestGamePcDocumentRemoveAllView(TokenAuthRequestMixin):
    """Tests for POST /games/<slug>/pcs/<id>/documents/remove/all.json."""

    def setup_method(self):
        """Set up a game, a DM, a PC with an owning player, and a hidden owned document."""
    ...
    def _url(self, character_id=None, game_slug=None):
        ...
        return f'/games/{game_slug}/pcs/{character_id}/documents/remove/all.json'
    ...
    def _post(self, client, token=None, character_id=None, game_slug=None):
        ...
        return self.post(
            ...
            {'game_document_id': self.game_document.id}, token=token,
        )

    def test_dm_can_remove_a_hidden_document(self, client):
        """Test that a DM can remove a hidden document on behalf of a PC via the all-variant."""
    ...
    def test_superuser_can_remove_a_hidden_document(self, client):
        """Test that a superuser can remove a hidden document via the all-variant."""
        superuser = SuperUserFactory(username='admin', password='secret-password')
        token = Token.objects.create(user=superuser)
        response = self._post(client, token=token)
        assert response.status_code == 204

    def test_owning_player_can_remove_a_hidden_document(self, client):
        """Test that a PC's own owning player can remove a hidden document via the all-variant."""
