"""Tests for the dm/admin/owner-only PC document ownership summary endpoint."""
...
class TestGamePcDocumentSummaryAllView(TokenAuthRequestMixin):
    """Tests for GET /games/<slug>/documents/<document_id>/pcs/<character_id>/summary/all.json."""

    def setup_method(self):
        """Set up a game, an owning player, a DM, an unrelated user, and a game document."""
    ...
    def _url(self, document_id=None, character_id=None, game_slug=None):
        ...
        return f'/games/{game_slug}/documents/{document_id}/pcs/{character_id}/summary/all.json'
...
@pytest.mark.django_db
class TestGamePcDocumentSummaryAllHiddenDocument(TokenAuthRequestMixin):
    """Tests for the /all.json summary endpoint against a hidden (GameDocument.hidden) document."""

    def setup_method(self):
        """Set up a game, a DM, an owning PC, and a hidden document the PC owns."""
        self.game = GameFactory(name='Test Game', game_slug='test-game')
        self.dm_user = UserFactory(username='dm_user', password='secret-password')
        PlayerFactory(game=self.game, user=self.dm_user, is_dm=True)
        self.dm_token = Token.objects.create(user=self.dm_user)
    ...
    def test_owner_can_view_ownership_of_a_hidden_document(self, client):
        """Test that the PC's owning player also bypasses the hidden-document gate."""
        response = self.get(client, self._url(), token=self.owner_token)
        assert response.status_code == 200
        assert json.loads(response.content) == {'owned': True}
