"""Tests for the DM-only PC document acquire-all endpoint (accepts hidden game documents)."""
...
class TestGamePcDocumentAcquireAllView(TokenAuthRequestMixin):
    """Tests for POST /games/<slug>/pcs/<id>/documents/acquire/all.json."""

    def setup_method(self):
        """Set up a game, a DM, a PC (with an owning player), and a hidden game document."""
    ...
    def _url(self, character_id=None, game_slug=None):
        ...
        return f'/games/{game_slug}/pcs/{character_id}/documents/acquire/all.json'
    ...
    def test_dm_can_acquire_a_hidden_document_on_behalf_of_a_pc(self, client):
        """Test that a DM can acquire a hidden document on behalf of a PC via the all-variant."""
