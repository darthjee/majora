"""Tests for the PC document ownership summary endpoint (open to everyone)."""
...
class TestGamePcDocumentSummaryView(TokenAuthRequestMixin):
    """Tests for GET /games/<slug>/documents/<document_id>/pcs/<character_id>/summary.json."""

    def setup_method(self):
        """Set up a game, a PC, and a game document."""
    ...
    def _url(self, document_id=None, character_id=None, game_slug=None):
        ...
        return f'/games/{game_slug}/documents/{document_id}/pcs/{character_id}/summary.json'

    def test_returns_false_when_not_owned(self, client):
        """Test that owned is False when the PC does not own the document."""
        response = self.get(client, self._url())
        assert response.status_code == 200
        assert json.loads(response.content) == {'owned': False}

    def test_returns_true_when_owned(self, client):
        """Test that owned is True when a CharacterDocument row links the PC to the document."""
...
@pytest.mark.django_db
class TestGamePcDocumentSummaryHiddenDocument(TokenAuthRequestMixin):
    """Tests for the summary endpoint against a hidden (GameDocument.hidden) document."""

    def setup_method(self):
        """Set up a game, a DM, a visible PC, and a hidden document."""
        self.game = GameFactory(name='Test Game', game_slug='test-game')
