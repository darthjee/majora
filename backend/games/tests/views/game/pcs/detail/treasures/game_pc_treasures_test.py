...
class TestGamePcTreasuresView(TokenAuthRequestMixin):
    ...
    def _url(self, character_id=None, game_slug='test-game'):
        ...
        return f'/games/{game_slug}/pcs/{character_id}/treasures.json'
    ...
    def test_ignores_non_numeric_min_value(self, client):
        ...
        CharacterTreasure.objects.create(character=self.character, treasure=treasure, quantity=1)
        response = client.get(f'{self._url()}?min_value=not-a-number')
        assert response.status_code == 200
        data = json.loads(response.content)
        assert len(data) == 1
