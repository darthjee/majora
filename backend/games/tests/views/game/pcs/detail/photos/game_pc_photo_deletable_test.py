"""Tests for the PC photo deletable-check endpoint."""
...
class TestGamePcPhotoDeletableView(TokenAuthRequestMixin):
    """Tests for GET /games/<slug>/pcs/<character_id>/photos/<photo_id>/deletable.json."""

    def setup_method(self):
        """Set up a game, an owning player, a DM, a staff user, a superuser, and a photo."""
    ...
    def _url(self, character_id=None, photo_id=None):
        ...
        return f'/games/epic-quest/pcs/{character_id}/photos/{photo_id}/deletable.json'
    ...
    def test_owning_player_returns_403(self, client):
        """Test that the owning player is rejected with 403."""
        response = self._get(client, token=self.owner_token)
    ...
    def test_unknown_game_slug_returns_404(self, client):
        ...
        response = client.get(
            f'/games/unknown-game/pcs/{self.character.id}/photos/{self.photo.id}/deletable.json',
...
@pytest.mark.django_db
class TestGamePcPhotoDeletableHiddenCharacter(TokenAuthRequestMixin):
    """Tests confirming the deletable-check endpoint ignores a hidden PC's hidden state."""

    def setup_method(self):
        """Set up a game, a hidden PC, its owning player, a DM, and a photo."""
        ...
        self.player.save()
        self.character = CharacterFactory(
            name='Secret PC', game=self.game, player=self.player, npc=False, hidden=True,
        )
        self.photo = CharacterPhoto.objects.create(
    ...
    def _url(self):
        ...
        return f'/games/test-game/pcs/{self.character.id}/photos/{self.photo.id}/deletable.json'

    def test_dm_can_check_deletable_for_hidden_pc(self, client):
        """Test that a DM can check deletability for a hidden PC's photo."""
        response = self.get(client, self._url(), token=self.dm_token)
        assert response.status_code == 200

    def test_owning_player_returns_403_for_hidden_pc(self, client):
        """Test that the owning player is rejected with 403 even for their own hidden PC."""
        owner_token = Token.objects.create(user=self.owner)
        response = self.get(client, self._url(), token=owner_token)
        assert response.status_code == 403

    def test_anonymous_returns_401_for_hidden_pc(self, client):
        """Test that an anonymous request for a hidden PC's photo returns 401."""
