...
class TestGamePcFactionRemoveView(TokenAuthRequestMixin):
    """Tests for POST /games/<slug>/pcs/<id>/factions/<faction_id>/remove.json."""
    ...
    def _url(self, faction_id=None, character_id=None, game_slug=None):
        ...
        return f'/games/{game_slug}/pcs/{character_id}/factions/{faction_id}/remove.json'
    ...
    def test_remove_deletes_character_faction_row(self, client):
        ...
        self._post(client, token=self._editor_token())
    ...
    def test_removing_never_enlisted_faction_returns_404(self, client):
        ...
        response = self._post(
            client, token=self._editor_token(), faction_id=other_faction.id,
        )
    ...
    def test_removing_hidden_enlisted_faction_returns_404_via_public_endpoint(self, client):
        ...
        response = self._post(client, token=self.dm_token, faction_id=hidden_faction.id)
        assert response.status_code == 404
    ...
    def test_unknown_game_slug_returns_404(self, client):
        ...
        response = self._post(client, token=self._editor_token(), game_slug='no-such-game')
        assert response.status_code == 404

    def test_unknown_character_id_returns_404(self, client):
        """Test that a non-existent character_id returns 404."""
        response = self._post(client, token=self._editor_token(), character_id=99999)
        assert response.status_code == 404
    ...
    def test_opposite_role_id_returns_404(self, client):
        ...
        response = self._post(client, token=self._editor_token(), character_id=other.id)
        assert response.status_code == 404
    ...
    def test_url_by_name(self, client):
        ...
        response = self.post(client, url, {}, token=self._editor_token())
        assert response.status_code == 204
    ...
    def test_plain_player_can_remove_faction(self, client):
        ...
        response = self._post(client, token=self.plain_player_token)
        assert response.status_code == 204
