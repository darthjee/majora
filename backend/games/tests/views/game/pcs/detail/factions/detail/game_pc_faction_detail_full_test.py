"""Tests for the PC faction detail/full.json view (dm, owner, or admin only; incl. hidden)."""
...
class TestGamePcFactionDetailFullView(TokenAuthRequestMixin):
    """Tests for GET /games/<slug>/pcs/<id>/factions/<faction_id>/full.json."""

    def setup_method(self):
        """Set up a game, an owning player/user, a DM, an unrelated user, and a hidden faction."""
    ...
    def _url(self, faction_id=None, character_id=None, game_slug='test-game'):
        ...
        return f'/games/{game_slug}/pcs/{character_id}/factions/{faction_id}/full.json'
    ...
    def test_owner_gets_200_for_hidden_faction(self, client):
        """Test that the PC's owning player gets 200 for a hidden faction."""
        response = self.get(client, self._url(), token=self.owner_token)
        assert response.status_code == 200
        data = json.loads(response.content)
        assert data['name'] == 'Hidden Faction'
    ...
    def test_returns_404_for_unknown_faction(self, client):
        ...
        response = self.get(client, self._url(faction_id=99999), token=self.owner_token)
