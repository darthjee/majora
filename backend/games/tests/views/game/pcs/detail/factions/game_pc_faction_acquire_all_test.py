"""Tests for the DM-only PC faction acquire-all endpoint (bypasses hidden-character gate)."""
...
class TestGamePcFactionAcquireAllView(TokenAuthRequestMixin):
    """Tests for POST /games/<slug>/pcs/<id>/factions/acquire/all.json."""

    def setup_method(self):
        """Set up a game, a DM, a PC (with an owning player), and a game faction."""
    ...
    def _url(self, character_id=None, game_slug=None):
        ...
        return f'/games/{game_slug}/pcs/{character_id}/factions/acquire/all.json'
    ...
    def test_dm_can_acquire_a_faction_on_behalf_of_a_pc(self, client):
        """Test that a DM can enlist a PC into a faction via the all-variant."""
        response = self._post(client, token=self.dm_token)
        assert response.status_code == 201

    def test_superuser_can_acquire_a_faction(self, client):
        """Test that a superuser can enlist a PC into a faction via the all-variant."""
