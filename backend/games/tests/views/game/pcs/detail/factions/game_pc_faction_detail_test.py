"""Tests for the PC faction detail view (GET)."""
...
class TestGamePcFactionDetailView(TokenAuthRequestMixin):
    """Tests for the GET /games/<slug>/pcs/<id>/factions/<faction_id>.json endpoint."""
    ...
    def _url(self, faction_id, character_id=None, game_slug='test-game'):
        ...
        return f'/games/{game_slug}/pcs/{character_id}/factions/{faction_id}.json'
    ...
    def test_returns_404_for_unknown_character(self, client):
        ...
        response = client.get(self._url(character_faction.id, character_id=99999))
        assert response.status_code == 404
    ...
    def test_returns_404_for_opposite_role_id(self, client):
        ...
        game_faction = GameFactionFactory(game=self.game, name='The Silver Hand')
        character_faction = CharacterFaction.objects.create(
            character=other, game_faction=game_faction,
        )
        response = client.get(self._url(character_faction.id, character_id=other.id))
        assert response.status_code == 404
