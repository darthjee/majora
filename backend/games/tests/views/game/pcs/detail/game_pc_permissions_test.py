"""Tests for the PC permissions-check view."""
...
class TestGamePcPermissionsView(TokenAuthRequestMixin):
    ...
    def _url(self, character=None, game_slug='test-game', query=''):
        ...
        url = f'/games/{game_slug}/pcs/{character.id}/permissions.json'
        if query:
            url = f'{url}?{query}'
        return url
    ...
    def test_owner_can_edit(self, client):
        ...
        response = self.get(client, self._url(query='role=owner'), token=token)
        data = json.loads(response.content)
        assert data == {
            'can_edit': True,
            'can_create_item': True,
