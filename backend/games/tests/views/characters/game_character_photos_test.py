"""Tests for the PC/NPC photos view."""
...
class _BaseCharacterPhotosViewTest(TokenAuthRequestMixin):
    """Shared behavior for GET .../photos.json (PC and NPC)."""
    ...
    def _url(self, character_id=None, game_slug='test-game'):
        ...
        return f'/games/{game_slug}/{self.segment}/{character_id}/photos.json'
    ...
    def test_returns_id_and_path_fields(self, client):
        ...
        photo = CharacterPhoto.objects.create(
            path=f'photos/games/test-game/characters/1/{self.character_name.lower()}.png',
            character=self.character,
            ready=True,
        )
        response = client.get(self._url())
...
class TestGameNpcPhotosView(_BaseCharacterPhotosViewTest):
    """Tests for the GET /games/<slug>/npcs/<id>/photos.json endpoint."""

    npc = True
    segment = 'npcs'
    url_name = 'game-npc-photos'
    character_name = 'Gandalf'


@pytest.mark.django_db
class TestGamePcPhotosView(_BaseCharacterPhotosViewTest):
    """Tests for the GET /games/<slug>/pcs/<id>/photos.json endpoint."""

    npc = False
    segment = 'pcs'
    url_name = 'game-pc-photos'
