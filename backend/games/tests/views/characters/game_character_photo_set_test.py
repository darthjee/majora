"""Tests for the PC/NPC photo set (role update) endpoint."""
...
class _BaseCharacterPhotoSetViewTest(TokenAuthRequestMixin):
    """Shared behavior for PATCH .../photos/<photo_id>/set.json (PC and NPC)."""

    npc = None
    segment = None

    def _editor_token(self):
        """Return a token belonging to a user allowed to set the character's photo."""
        raise NotImplementedError

    def _opposite_role_character(self):
        """Create and return a character of the opposite role (PC for NPC tests, vice versa)."""
        return CharacterFactory(name='Other', game=self.game, npc=not self.npc)
    ...
    def _url(self, character_id=None, photo_id=None):
        ...
        return f'/games/epic-quest/{self.segment}/{character_id}/photos/{photo_id}/set.json'
    ...
    def test_photo_of_another_character_returns_404(self, client):
        ...
        other_character = CharacterFactory(name='Legolas', game=self.game, npc=self.npc)
...
class TestGameNpcPhotoSetView(_BaseCharacterPhotoSetViewTest):
...
@pytest.mark.django_db
class TestGamePcPhotoSetView(_BaseCharacterPhotoSetViewTest):
