"""Tests for the PC/NPC photo upload init endpoint."""
...
class _BaseCharacterPhotoUploadViewTest(TokenAuthRequestMixin):
    """Shared behavior for POST .../photo_upload.json (PC and NPC)."""

    npc = None
    segment = None

    def _editor_token(self):
        """Return a token belonging to a user allowed to upload a photo for the character."""
    ...
    def _url(self, character_id=None):
        ...
        return f'/games/epic-quest/{self.segment}/{character_id}/photo_upload.json'
...
class TestGameNpcPhotoUploadView(_BaseCharacterPhotoUploadViewTest):
