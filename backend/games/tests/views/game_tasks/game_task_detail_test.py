...
class TestGameTaskDetailPatchView(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.task = Task.objects.create(game=cls.game, short_description='Old description')
    ...
    def test_superuser_can_patch(self):
        ...
        response = self._patch(
            self.client, {'short_description': 'New description'}, token=self.superuser_token
        )
        assert response.status_code == 200
    ...
    def test_patch_without_token_returns_401(self):
        ...
        response = self._patch(self.client, {'short_description': 'Hacked'})
        assert response.status_code == 401

    def test_patch_with_non_game_master_returns_403(self):
        """Test that PATCH from a non-DM, non-superuser returns 403."""
        response = self._patch(
            self.client, {'short_description': 'Hacked'}, token=self.regular_token
        )
        assert response.status_code == 403

    def test_patch_non_existent_task_returns_404(self):
        """Test that PATCH on a non-existent task returns 404."""
        response = self._patch(
            self.client, {'short_description': 'Ghost'}, token=self.dm_token, task_id=999999,
        )
        assert response.status_code == 404

    def test_patch_returns_404_for_unknown_game_slug(self):
        """Test that 404 is returned for a non-existent game slug."""
        response = self._patch(
            self.client, {'short_description': 'Ghost'}, token=self.dm_token,
            game_slug='unknown-game',
        )
        assert response.status_code == 404

    def test_patch_returns_404_when_task_belongs_to_different_game(self):
        """Test that 404 is returned when task_id does not belong to game_slug."""
        response = self._patch(
            self.client, {'short_description': 'Hacked'}, token=self.dm_token,
    ...
    def test_patch_partial_body_only_changes_given_fields(self):
        ...
        self._patch(self.client, {'short_description': 'Partial Update'}, token=self.dm_token)
        self.task.refresh_from_db()
    ...
    def test_patch_can_set_session(self):
        ...
        response = self._patch(self.client, {'session': session.id}, token=self.dm_token)
        assert response.status_code == 200
        self.task.refresh_from_db()
    ...
    def test_patch_can_clear_session(self):
        ...
        self.task.save()
        response = self._patch(self.client, {'session': None}, token=self.dm_token)
        assert response.status_code == 200
        self.task.refresh_from_db()
    ...
    def test_patch_session_from_different_game_returns_400(self):
        ...
        response = self._patch(self.client, {'session': other_session.id}, token=self.dm_token)
        assert response.status_code == 400
        data = json.loads(response.content)
        assert 'session' in data['errors']

    def test_patch_game_field_in_payload_is_ignored(self):
