...
def character_full(request, game, character_id, npc):
    ...
    error_response = _check_character_edit(request, character)
    if error_response:
    ...
    response = detail_or_update(
        request,
        character,
        _check_character_edit,
        CharacterUpdateSerializer,
        CharacterFullSerializer,
        detail_context={'request': request},
    )
    # The entire response (GET and PATCH) is gated behind _check_character_edit()
