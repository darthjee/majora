"""Game possession permissions serializer for the games app."""

from games.serializers.base_permissions import BasePermissionsSerializer
from permissions import PermissionsBuilder

_PAGE_KEY = 'game_possession'


class GamePossessionPermissionsSerializer(BasePermissionsSerializer):
    """Serializes the can_edit flag for the entity-agnostic game possession permissions.json."""

    def to_representation(self, instance):
        """Build the permissions response dict via the YAML-driven `PermissionsBuilder`.

        `instance` is always `None` (the entity-agnostic `/permissions/game_possession.json`
