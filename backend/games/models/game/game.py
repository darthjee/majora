...
class Game(models.Model):
    ...
    def has_player(self, user, is_dm=None):
        ...
        Delegates to `GamePlayerCache`, which caches the result in the shared process-wide
        memory cache, keyed by user id, game id, and the `is_dm` filter, since this is called
        both without a filter (any player or DM) and with `is_dm=True` (DM only), and those
        two calls must not share a cached result.
        """
