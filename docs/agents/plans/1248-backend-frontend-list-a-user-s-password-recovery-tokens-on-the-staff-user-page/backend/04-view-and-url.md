...
New `staff/views/staff_user_recovery_tokens.py`, following `staff/views/staff_user_recovery_link.py`'s pattern verbatim (same decorator stack, `require_staff` gate, `get_object_or_404`).
...
`user.password_reset_tokens` is the existing `related_name` on `PasswordResetToken.user`.

Register the route in `staff/urls.py`, right after `recovery-link.json` (same nesting depth as #1249's future mutation routes):
...
- `backend/staff/views/staff_user_recovery_tokens.py` — new.
- `backend/staff/views/__init__.py` — export `staff_user_recovery_tokens`.
- `backend/staff/urls.py` — register the `GET .../recovery-tokens.json` route.
