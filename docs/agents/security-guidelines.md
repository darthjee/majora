...
- **Exception — inline `require_authenticated`/`require_staff` pattern:** views that need to return a custom 401 (unauthenticated) vs. 403 (authenticated but not staff/superuser) body, rather than DRF's default permission-denied response, declare `@permission_classes([AllowAny])` with a comment noting that auth/authorization is enforced inline, then call `require_authenticated`/`require_staff` (`backend/games/views/common.py`) as the first statement in the view body, returning its error response immediately if non-`None`. This is an accepted alternative to a declarative permission class, not an authentication gap — see `backend/games/views/staff/staff_users_list.py` and `backend/accounts/views/auth/email.py` for reference implementations. Any other use of `AllowAny` must still be justified as truly public per the rule above.
