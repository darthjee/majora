...
`can_edit` follows the shared `GameEdit` rule (dm/admin only); `can_create_item` is backed by
...
is computed identically, but backed by `GameDocumentCreatePermission` instead — see
