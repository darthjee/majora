...
(cursor pagination, retention cap, request/response shapes). The `POST`, `GET`, and `DELETE`
on `/staff/crawler.json` plus the `GET` on `/staff/crawler/summary.json` all enforce
**Staff-or-superuser** inline (via `require_staff`), matching every other `staff/*` endpoint.
All four responses set `X-Skip-Cache: true` per the
...
summary `GET` returns per-`type` row counts (`{}` when the table is empty); the `DELETE` is a
blanket clear of every row, not scoped by `type`/`source`. The model, migration, and these
endpoints are all deleted once #1262's real import endpoint is trusted end-to-end (see the
spec's "Removal plan").
