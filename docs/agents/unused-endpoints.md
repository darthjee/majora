...
|----------|------------|------------|------------|
| `PATCH /uploads/(image\|file)/<id>.json` | `games` (`games.views.upload_finalize`) | Long-standing, predates focused issue tracking | The upload lifecycle's finalize/status-transition step. `UploadClient` (frontend) only performs the init (`POST .../photo_upload.json`) and submit (`POST /uploads/<type>/<id>/submit`, handled by the PHP proxy for multipart) steps — the JSON endpoint that advances the upload's status and marks the linked photo/file "ready" is never called from the frontend. |
