...
`null`, and its own independent `hidden` flag. `GameItem` gained a photo upload endpoint (issue #749);
`CharacterItem` gained both a create endpoint (issue #714) and a photo upload endpoint
