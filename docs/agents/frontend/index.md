# Front-End Architecture

This folder contains the split sections for the original `docs/agents/frontend.md` file.

Files

- runtime-boot-flow.md — Runtime Boot Flow
- running-locally.md �� Running Locally
- directory-structure.md — Directory Structure
- component-architecture.md — Component Architecture & Avoiding Inline JSX Conditionals
- pages-elements.md — Pages vs Elements / Adding pages & elements
- routing-pagination.md — Routing and Pagination
- api-client-requests.md — API Client and Resource Data Requests
- logging.md — Logging
- bootstrap-linting-tests.md — Bootstrap, Linting & Tests

Each file preserves the original headings and contents from the consolidated document.
