...
| `external/tent/` | Per-topic Tent proxy reference pages (`setup.md`, `request-handlers.md`, `host-header.md`, `middlewares.md`, `cache-configuration.md`, `dev-mode-and-static.md`, `complete-example.md`, `extending.md`, `reference.md`). Split out of the former monolithic `how-to-use-tent.md` so an agent only loads the page it needs. |
