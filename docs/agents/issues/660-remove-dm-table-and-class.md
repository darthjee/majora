...
- Remove the `GameMaster` model, its serializer, its admin registration, and its dedicated views/URLs (`game_masters_list`, `game_master_detail`, and the `games/<slug>/game-masters(.json|/<id>.json)` routes) entirely — they're unused by the frontend today and have no reason to survive without a backing table.
  - Note: these endpoints are currently the only way to add/remove a DM on an *existing* game (self-assign, list, step down) — no other code path writes `Player.is_dm` after game creation. Removing them accepts that gap; no replacement mechanism ships in this issue. Reintroducing self-service DM promotion/demotion (via `Player.is_dm`) is left for a future issue if ever needed.
...
- Update product/access-control docs referencing `GameMaster` as the source of DM status: `docs/agents/product.md` ("GameMaster Role" section, "Editing Rules" rule 3, "User" and "Staff Role" sections), `docs/agents/access-control/common-rules.md` (`can_be_edited_by` derivations, `is_dm` field description), `docs/agents/access-control/user-roles.md` (GameMaster role definition), `docs/agents/models-organization.md` and `docs/agents/views-organization.md` (drop removed model/view-folder references). Delete `docs/agents/access-control/game-master.md` (documents the removed endpoints) and its link from `docs/agents/access-control.md`'s index.
