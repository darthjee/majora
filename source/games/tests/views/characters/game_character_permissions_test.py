"""Tests for the PC/NPC permissions-check view."""
...
class _BaseCharacterPermissionsViewTest(TokenAuthRequestMixin):
    """Shared behavior for the PC and NPC permissions-check endpoints."""
    ...
    def _url(self, character=None, game_slug='test-game', query=''):
        ...
        url = f'/games/{game_slug}/{self.segment}/{character.id}/permissions.json'
        if query:
            url = f'{url}?{query}'
        return url
    ...
    def test_returns_200_with_can_edit_false_for_unknown_character(self, client):
        ...
        response = client.get(f'/games/test-game/{self.segment}/99999/permissions.json')
...
class TestGameNpcPermissionsView(_BaseCharacterPermissionsViewTest):
...
@pytest.mark.django_db
class TestGamePcPermissionsView(_BaseCharacterPermissionsViewTest):
    """Tests for the PC permissions endpoint."""
