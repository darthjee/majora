...
class TestGamePcPhotosView:
    ...
    def test_returns_empty_list_when_no_photos(self, client):
        """Test that an empty list is returned when the PC has no photos."""
        response = client.get(f'/games/test-game/pcs/{self.character.id}/photos.json')
    ...
    def test_returns_id_and_path_fields(self, client):
        ...
        assert data[0]['path'] == 'photos/games/test-game/characters/1/aragorn.png'

    def test_returns_404_for_unknown_character(self, client):
        """Test that 404 is returned for a non-existent character_id."""
        response = client.get('/games/test-game/pcs/99999/photos.json')
    ...
    def test_returns_404_for_character_in_wrong_game(self, client):
        ...
        response = client.get(f'/games/other-game/pcs/{self.character.id}/photos.json')
        assert response.status_code == 404
    ...
    def test_returns_404_for_npc_id(self, client):
        ...
        response = client.get(f'/games/test-game/pcs/{npc.id}/photos.json')
        assert response.status_code == 404
    ...
    def test_response_includes_per_page_header(self, client):
        ...
        response = client.get(
            f'/games/test-game/pcs/{self.character.id}/photos.json?per_page=5'
        )
    ...
    def test_respects_page_param(self, client):
        ...
        response = client.get(
            f'/games/test-game/pcs/{self.character.id}/photos.json?page=2&per_page=3'
        )
        assert response.status_code == 200
        data = json.loads(response.content)
        assert len(data) == 2
