...
class TestGamePermissionsView(TokenAuthRequestMixin):
    ...
    def test_non_existent_slug_returns_200_with_can_edit_false(self, client):
        ...
        response = client.get('/games/no-such-game/permissions.json')
    ...
    def test_non_dm_user_cannot_edit(self, client):
    ...
    def test_role_player_alone_cannot_edit(self, client):
        """Test that ?role=player alone does not grant can_edit."""
        response = self._get(client, query='role=player')
        data = json.loads(response.content)
        assert data == {'can_edit': False}

    def test_role_dm_overrides_authenticated_non_dm_real_identity(self, client):
        """Test that ?role=dm grants can_edit True even when the real caller is not a DM."""
        other = UserFactory(username='other', password='secret-password')
        token = Token.objects.create(user=other)
        response = self._get(client, token=token, query='role=dm')
        data = json.loads(response.content)
        assert data == {'can_edit': True}
    ...
    def test_unrecognized_role_does_not_fall_back_to_real_identity(self, client):
        ...
        token = Token.objects.create(user=self.dm_user)
        response = self._get(client, token=token, query='role=bogus')
        data = json.loads(response.content)
        assert data == {'can_edit': False}
    ...
    def test_role_simulated_response_is_publicly_cacheable_for_authenticated_caller(self, client):
        """Test that Cache-Control stays public even when the real caller is authenticated."""
        token = Token.objects.create(user=self.dm_user)
        response = self._get(client, token=token, query='role=owner')
