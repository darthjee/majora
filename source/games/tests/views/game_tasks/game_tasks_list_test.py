...
class TestGameTasksListView:
    ...
    def test_non_game_master_get_returns_403(self, client):
    ...
    def test_returns_404_for_unknown_game_slug(self, client):
        ...
        response = self._get(client, token=self.dm_token, game_slug='unknown-game')
    ...
    def test_respects_per_page_param(self, client):
        ...
        response = self._get(client, token=self.dm_token, query='?per_page=2')
        assert response.status_code == 200
        data = json.loads(response.content)
        assert len(data) == 2
    ...
    def test_respects_page_param(self, client):
        ...
        for i in range(5):
            Task.objects.create(game=self.game, short_description=f'Task {i}')
        response = self._get(client, token=self.dm_token, query='?page=2&per_page=3')
        assert response.status_code == 200
        data = json.loads(response.content)
        assert len(data) == 2
    ...
    def test_url_by_name(self, client):
        ...
        response = client.get(url, HTTP_AUTHORIZATION=f'Token {self.dm_token.key}')
        assert response.status_code == 200
...
class TestGameTasksCreateView:
    ...
    def test_game_master_can_create_task(self, client):
        """Test that a DM of the game can create a task and receives 201."""
        response = self._post(client, {'short_description': 'Prep the ambush'}, token=self.dm_token)
        assert response.status_code == 201

    def test_superuser_can_create_task(self, client):
        """Test that a superuser can create a task and receives 201."""
        response = self._post(
            client, {'short_description': 'Prep the ambush'}, token=self.superuser_token
        )
        assert response.status_code == 201

    def test_create_returns_task_detail(self, client):
    ...
    def test_unauthenticated_post_returns_401(self, client):
        ...
        response = self._post(client, {'short_description': 'Prep the ambush'})
        assert response.status_code == 401
        data = json.loads(response.content)
        assert 'detail' in data['errors']
    ...
    def test_non_game_master_post_returns_403(self, client):
        ...
        response = self._post(
            client, {'short_description': 'Prep the ambush'}, token=self.regular_token
        )
        assert response.status_code == 403
        data = json.loads(response.content)
        assert 'detail' in data['errors']

    def test_missing_short_description_returns_400(self, client):
        """Test that a POST without short_description returns 400."""
        response = self._post(client, {'long_description': 'Some notes'}, token=self.dm_token)
        assert response.status_code == 400
        data = json.loads(response.content)
        assert 'short_description' in data['errors']

    def test_session_from_different_game_returns_400(self, client):
    ...
    def test_post_returns_404_for_unknown_game_slug(self, client):
        ...
        response = self._post(
            client, {'short_description': 'Prep the ambush'}, token=self.dm_token,
            game_slug='unknown-game',
        )
        assert response.status_code == 404

    def test_created_task_persists_to_database(self, client):
        """Test that the created task is persisted and linked to the game."""
        self._post(client, {'short_description': 'Prep the ambush'}, token=self.dm_token)
        assert Task.objects.filter(game=self.game, short_description='Prep the ambush').exists()

    def test_game_field_in_payload_is_ignored(self, client):
