...
class TestGameNpcDetailView:
    ...
    def test_returns_404_for_pc_id(self, client):
        ...
        pc = Character.objects.create(name="Aragorn", game=self.game, player=self.player, npc=False)
    ...
    def test_role_is_null_when_not_set(self, client):
        ...
        npc = Character.objects.create(name="Unnamed NPC", game=self.game, role=None, npc=True)
...
class TestGamePcDetailView:
    ...
    def test_includes_photos(self, client):
        """Test that character detail includes associated photos."""
        Photo.objects.create(url="http://example.com/aragorn.png", character=self.character)
        response = client.get(f"/games/test-game/pcs/{self.character.id}.json")
        data = json.loads(response.content)
        assert len(data["photos"]) == 1
