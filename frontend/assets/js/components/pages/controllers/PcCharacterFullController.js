...
/**
 * Extract game slug and character id from a PC character full hash.
 *
 * @param {string} hash - Current hash.
 * @returns {object} Character route params.
 */
export function getPcCharacterFullParamsFromHash(hash = '') {
  const params = Router.extractParams('/games/:game_slug/pcs/:character_id/full', hash);
...
/**
 * Controller for the PC character full detail page.
 */
export default class PcCharacterFullController extends BasePageController {
  /**
   * Create a PC character full controller.
   *
  ...
  buildEffect() {
    return () => {
      ...
      const params = getPcCharacterFullParamsFromHash(this.client.currentHash());
      ...
      if (!params.game_slug || !params.character_id) {
        ...
        this.characterClient.fetchPcFull(params.game_slug, params.character_id, token)
          .then((response) => {
            if (response.status === 403) {
              PcCharacterFullController.#redirectToDetail(params.game_slug, params.character_id);
              return null;
            }
