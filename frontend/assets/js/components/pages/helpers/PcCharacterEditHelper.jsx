...
export default class PcCharacterEditHelper {
  ...
  static render(state, handlers) {
    return (
      <div className="container mt-4">
        <h1>{Translator.t('pc_edit_page.title')}</h1>
        {PcCharacterEditHelper.#renderError(state)}
        <div className="row">
          ...
          <div className="col-md-8">
            <form onSubmit={handlers.onSubmit}>
              <FormField
                id="pc-edit-name"
                type="text"
                label={Translator.t('pc_edit_page.name_label')}
                value={state.name}
                onChange={handlers.onNameChange}
                errors={state.fieldErrors.name ?? []}
              />
              <FormField
                id="pc-edit-avatar-url"
                type="text"
                label={Translator.t('pc_edit_page.avatar_url_label')}
                value={state.avatar_url}
                onChange={handlers.onAvatarUrlChange}
                errors={state.fieldErrors.avatar_url ?? []}
              />
              <FormField
                id="pc-edit-character-class"
                type="text"
                label={Translator.t('pc_edit_page.character_class_label')}
              ...
              <FormField
                id="pc-edit-level"
                type="number"
                label={Translator.t('pc_edit_page.level_label')}
                value={state.level}
                onChange={handlers.onLevelChange}
                errors={state.fieldErrors.level ?? []}
              />
              <FormField
                id="pc-edit-description"
                type="text"
  ...
  static #renderError(state) {
    ...
    return <ErrorAlert error={Translator.t('pc_edit_page.error')} />;
  }
}
