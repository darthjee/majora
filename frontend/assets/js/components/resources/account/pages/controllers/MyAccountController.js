...
export default class MyAccountController extends BasePageController {
   ...
   * @param {Function} setDisplayName - Display name setter, used to prefill the form.
   ...
   *   email: string, password: string, passwordConfirmation: string}} formValues - Raw form
   *   field values.
  ...
  #fetchAccount(safeSet) {
    ...
    this.client.fetchAccount(token)
      .then((response) => parseJsonOrReject(response, 'account fetch failed'))
      .then((account) => {
