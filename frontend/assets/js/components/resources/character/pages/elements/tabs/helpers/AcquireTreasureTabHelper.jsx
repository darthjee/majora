...
/**
 * Rendering helper for the treasure exchange modal's Acquire tab: the paginated browse list of a
 ...
 */
export default class AcquireTreasureTabHelper {
  /**
   ...
   *   last acquire request was only partially fulfilled.
  ...
  static render(state, handlers) {
    ...
    return (
      <TwoColumnLayout
        browsePane={AcquireTreasureTabHelper.#renderBrowsePane(state, handlers)}
        detailPane={selected ? AcquireTreasureTabHelper.#renderDetailPane(state, handlers) : null}
      />
    );
  }
  ...
  static #renderBrowsePane(state, handlers) {
    return (
      <>
        {AcquireTreasureTabHelper.#renderPartialNotice(state.partialNotice)}
        {AcquireTreasureTabHelper.#renderSearchInput(state.search, handlers.onSearchChange)}
        <BrowsePager browse={state.browse} onPrev={handlers.onPrev} onNext={handlers.onNext} />
        {AcquireTreasureTabHelper.#renderBrowseList(state, handlers)}
      </>
    );
  }
        ...
        {browse.items.map((item) => (
          ...
          >
            ...
            <span className="d-flex align-items-center">
              {AcquireTreasureTabHelper.#renderAvailabilityBadge(item)}
