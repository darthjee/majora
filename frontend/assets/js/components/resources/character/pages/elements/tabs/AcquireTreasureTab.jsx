...
import AcquireTreasureTabController from './controllers/AcquireTreasureTabController.js';
import AcquireTreasureTabHelper from './helpers/AcquireTreasureTabHelper.jsx';
...
/**
 * Acquire tab of the treasure exchange modal: browses a game's full catalog treasures (paginated
 * using local component state, independent of the page's URL/pagination) and lets the character
 * be granted a quantity of the selected one, without touching its money.
 *
 ...
 *   money, acquired}` after a successful acquire action. `acquired` may be less than the
 *   requested quantity when the treasure was capped.
 * @returns {React.ReactElement} Rendered Acquire tab.
 */
export default function AcquireTreasureTab({
  ...
  const controller = useMemo(() => new AcquireTreasureTabController(), []);
  ...
  const handleConfirm = () => controller.confirmAcquire(selected, quantity, character, {
    setSubmitting,
    setSelected,
    setPartialNotice,
    setActionError,
  ...
  return AcquireTreasureTabHelper.render(
    {
