...
/**
 * Rendering helper for the treasure exchange modal's Sell tab: the paginated browse list of the
 ...
 */
export default class SellTreasureTabHelper {
  /**
   * Renders the Sell tab body.
   *
   ...
   * @param {number} state.quantity - Quantity to sell for the selected item.
   * @param {boolean} state.submitting - Whether a sell request is in flight.
   ...
   * @returns {React.ReactElement} Rendered Sell tab body.
   */
  static render(state, handlers) {
    ...
    return (
      <TwoColumnLayout
        browsePane={SellTreasureTabHelper.#renderBrowsePane(state, handlers)}
        detailPane={selected ? SellTreasureTabHelper.#renderDetailPane(state, handlers) : null}
      />
    );
  }
  ...
  static #renderBrowsePane(state, handlers) {
    return (
      <>
        {SellTreasureTabHelper.#renderSearchInput(state.search, handlers.onSearchChange)}
        <BrowsePager browse={state.browse} onPrev={handlers.onPrev} onNext={handlers.onNext} />
        {SellTreasureTabHelper.#renderBrowseList(state, handlers)}
      </>
    );
  }
