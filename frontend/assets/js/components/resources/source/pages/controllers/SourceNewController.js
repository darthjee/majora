...
/**
 * Controller for the "New Source" modal (`SourceNewModal.jsx`).
 *
 * @description Mirrors `StlModelNewController`'s deferred-photo-upload flow: the source is
 *   created first (name + url, no photo), then — if a photo was picked — a second saga step
 *   uploads it against the newly created id, mirroring `StlModelNewController#retryPhotoUpload`/
 *   `#failPhotoUpload`. There is no page-mount redirect gate here — the modal is only reachable
 *   through a button `SourcesHelper` already renders exclusively for staff/superuser viewers —
 ...
 *   "success" means.
 */
export default class SourceNewController extends BasePageController {
  /**
   * Create a source new controller.
   *
  ...
  /**
   * Submit the new source form.
   *
   * @description Prevents the default form submission, resets status and field errors, sends a
   *   POST request through {@link RequestStore.mutate} (so the source collection's cached `GET`
   ...
   *   source id) once creation, and its photo upload if any, has fully succeeded.
  ...
  /**
   * Retry the photo upload saga step for an already-created source.
   *
   * @description Re-invokes the same upload-only path `submitForm` runs after source creation,
   *   without creating a new source. Used by the "retry" action of the photo-upload-failed UI
   *   state.
   * @param {number|string} sourceId - Already-created source id.
   ...
   */
  retryPhotoUpload(sourceId, photoFile, setters) {
    return this.#uploadPhoto(sourceId, photoFile, setters);
  }
  ...
  async #uploadPhoto(sourceId, photoFile, setters) {
    const token = AuthStorage.getToken();
    const uploadPath = await RequestStore.resolvePath({
      resource: 'source', method: 'POST', quantityType: 'single', params: { id: sourceId },
    });
    ...
    if (ok) {
      ...
      // doesn't re-serve the pre-upload cached source.
      RequestStore.purge({ resource: 'source' });
      setters.onSuccess(sourceId);
      return;
    }

    this.#failPhotoUpload(sourceId, setters);
  }

  #failPhotoUpload(sourceId, setters) {
    setters.setCreatedId(sourceId);
