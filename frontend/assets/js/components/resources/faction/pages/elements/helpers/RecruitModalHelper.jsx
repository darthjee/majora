...
import RecruitReceivingRowHelper from './RecruitReceivingRowHelper.jsx';
...
/**
 * Rendering helper for the recruit modal (issue #943): a PC/NPC tab + search + browse-list left
 * pane, and (once at least one character has been picked) a right-side "receiving" list, laid out
 * through the shared {@link TwoColumnLayout} component — a 1:1 structural mirror of
 * `GiveDocumentModalHelper`. The receiving list collapses back to a single column automatically
 * whenever it becomes empty (either by removing the last row, or on `Clear`), matching
 * `TwoColumnLayout`'s own `detailPane` contract. No remaining-pool header badge — `CharacterFaction`
 * enlistment carries no quantity concept.
 */
export default class RecruitModalHelper {
  /**
   * Renders the recruit modal.
   *
    ...
    return (
      <Modal show={show} onHide={onHide}>
        <Modal.Header closeButton={!state.submitting}>
          <Modal.Title>{Translator.t('recruit_modal.title')}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <TwoColumnLayout
            browsePane={RecruitModalHelper.#renderBrowsePane(state, handlers)}
            detailPane={RecruitModalHelper.#renderDetailPane(state, handlers)}
          />
        </Modal.Body>
        <Modal.Footer>
          {RecruitModalHelper.#renderFooter(state, handlers)}
        </Modal.Footer>
      </Modal>
    );
  }
  ...
  static #renderFooter(state, handlers) {
    ...
    return (
      <>
        ...
        >
          {Translator.t('recruit_modal.submit')}
        </button>
      </>
    );
  }
  ...
  static #renderDetailPane(state, handlers) {
    ...
    return (
      <div className="list-group">
        {receiving.map((row) => RecruitReceivingRowHelper.render(row, handlers))}
      </div>
    );
  }
  ...
  static #renderBrowsePane(state, handlers) {
    return (
      <>
        {RecruitModalHelper.#renderTabs(state.activeTab, handlers.onTabChange)}
        {RecruitModalHelper.#renderSearchInput(state.search, handlers.onSearchChange)}
        <BrowsePager browse={state.browse} onPrev={handlers.onPrev} onNext={handlers.onNext} />
        {RecruitModalHelper.#renderBrowseList(state, handlers)}
      </>
    );
  }
      ...
      <ul className="nav nav-tabs mb-3">
        {TABS.map((tab) => (
          <li className="nav-item" key={tab}>
            ...
            >
              {Translator.t(`recruit_modal.${tab === 'pcs' ? 'pc_tab' : 'npc_tab'}`)}
            </button>
          </li>
        ))}
      </ul>
  ...
  static #renderSearchInput(search, onSearchChange) {
    return (
      <input
        ...
        placeholder={Translator.t('recruit_modal.search_placeholder')}
        value={search ?? ''}
  ...
  static #renderBrowseList(state, handlers) {
    ...
    if (browse.loading) {
      return <p className="text-muted">{Translator.t('recruit_modal.loading')}</p>;
    }
