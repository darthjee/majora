...
/**
 * Rendering helper for the "New Collection" modal: a single-column form (photo, then name, then
 * url) inside `Modal.Body`, with a full-width submit button as the `Modal.Footer`, mirroring
 * `SourceNewModalHelper`'s shell pattern. There is no two-column split and no `source` picker —
 * `Collection.source` starts `null` on create and is assigned later, a separate not-yet-built
 * feature. Reuses the `collection_new_page.*` i18n keys.
 */
export default class CollectionNewModalHelper {
   ...
   *   default `default_collection.png` placeholder).
