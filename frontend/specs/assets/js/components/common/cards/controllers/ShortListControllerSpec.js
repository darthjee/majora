...
describe('ShortListController', function() {
  ...
  describe('#buildEffect', function() {
    it('fetches a game-scoped resource (pc) through RequestStore', async function() {
      ...
      ensureSpy.and.returnValue(Promise.resolve({ data: pcs }));

      const cleanup = new ShortListController('pc', setItems, setLoading)
    ...
    it('fetches a character-scoped resource (treasure) using its configured buildParams', async function() {
      ...
      const cleanup = new ShortListController('treasure', setItems, setLoading)
    ...
    it('degrades to an empty list when the fetch rejects', async function() {
      ...
      const cleanup = new ShortListController('item', setItems, setLoading)
    ...
    it('degrades to an empty list when the resolved data is not an array', async function() {
      ensureSpy.and.returnValue(Promise.resolve({ data: { unexpected: 'shape' } }));

      const cleanup = new ShortListController('npc', setItems, setLoading)
        .buildEffect({ game_slug: 'demo' }, 5)();
      await new Promise((resolve) => setTimeout(resolve, 0));
    ...
    it('does not update state after unmount', async function() {
      ensureSpy.and.returnValue(Promise.resolve({ data: [{ id: 1 }] }));
