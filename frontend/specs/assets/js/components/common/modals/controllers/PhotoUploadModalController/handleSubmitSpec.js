...
describe('PhotoUploadModalController', function() {
  ...
  beforeEach(function() {
    ...
    client = {
      runUploadCycle: jasmine.createSpy('runUploadCycle'),
    };
  });

  describe('#handleSubmit', function() {
    it('calls onSuccess when the upload cycle succeeds', async function() {
      client.runUploadCycle.and.returnValue(Promise.resolve({
      ...
      expect(client.runUploadCycle).toHaveBeenCalledWith(
        '/games/my-game/photo_upload.json', file, 'auth-token', undefined,
      );
    ...
    it('forwards the given name to runUploadCycle (issue #874)', async function() {
      client.runUploadCycle.and.returnValue(Promise.resolve({
        ok: true, upload_id: 1, token: 'up-token', upload_type: 'file',
      }));
    ...
    it('sets error when the upload cycle does not succeed', async function() {
      client.runUploadCycle.and.returnValue(Promise.resolve({ ok: false }));
    ...
    describe('chained photo upload (issue #878)', function() {
      ...
      beforeEach(function() {
        client.runUploadCycle.and.returnValue(Promise.resolve({
          ok: true, upload_id: 1, token: 'up-token', upload_type: 'file', id: 42,
        }));
      });
      ...
      it('does not run a second cycle when no photoUpload is given', async function() {
        ...
        expect(client.runUploadCycle).toHaveBeenCalledTimes(1);
        expect(onSuccess).toHaveBeenCalled();
      });

      it('builds the photo path from the first cycle id and runs a second cycle', async function() {
        const buildPath = jasmine.createSpy('buildPath').and.returnValue(
          '/games/my-game/documents/9/files/42/photo_upload.json',
        );
        client.runUploadCycle.and.returnValues(
          Promise.resolve({
            ok: true, upload_id: 1, token: 'up-token', upload_type: 'file', id: 42,
          }),
          Promise.resolve({ ok: true, upload_id: 2, token: 'photo-token', upload_type: 'image' }),
        );
        ...
        expect(client.runUploadCycle).toHaveBeenCalledWith(
          '/games/my-game/documents/9/files/42/photo_upload.json', photoFile, 'auth-token',
        );
        expect(onSuccess).toHaveBeenCalled();
        expect(setUploading).toHaveBeenCalledWith(false);
      ...
      it('sets a distinct "photo" error when the second cycle does not succeed', async function() {
        client.runUploadCycle.and.returnValues(
          Promise.resolve({
            ok: true, upload_id: 1, token: 'up-token', upload_type: 'file', id: 42,
          }),
        ...
        expect(setUploading).toHaveBeenCalledWith(false);
        expect(client.runUploadCycle).toHaveBeenCalledTimes(2);
        expect(onSuccess).not.toHaveBeenCalled();
      });
