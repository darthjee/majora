import MoneyEditModalController
  from '../../../../../../../assets/js/components/common/modals/controllers/MoneyEditModalController.js';

describe('MoneyEditModalController', function() {
  describe('.denominationKeys', function() {
    it('returns the 4 character denomination keys by default', function() {
      expect(MoneyEditModalController.denominationKeys()).toEqual(['cp', 'sp', 'gp', 'pp']);
    });

    it('returns the 4 character denomination keys for the character context', function() {
      expect(MoneyEditModalController.denominationKeys('character')).toEqual(['cp', 'sp', 'gp', 'pp']);
    });
  ...
  describe('.seedBreakdown', function() {
    ...
    it('lets platinum absorb the remainder instead of overflowing into gems', function() {
      expect(MoneyEditModalController.seedBreakdown(32221)).toEqual({
        cp: 21, sp: 20, gp: 20, pp: 30,
      });
    });
  ...
  describe('.computeTotal', function() {
    it('sums copper, silver, gold and platinum weighted by their relative value', function() {
      ...
      expect(MoneyEditModalController.computeTotal(breakdown)).toBe(2 + 30 + 400 + 5000);
    });
