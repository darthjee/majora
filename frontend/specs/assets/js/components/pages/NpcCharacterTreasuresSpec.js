...
describe('NpcCharacterTreasures', function() {
  it('renders the loading state while fetching', function() {
    spyOn(NpcCharacterTreasuresController.prototype, 'buildEffect').and.returnValue(() => Noop.noop);
    spyOn(CharacterTreasuresHelper, 'renderLoading').and.returnValue(React.createElement('div', null, 'loading'));

    const html = renderToStaticMarkup(React.createElement(NpcCharacterTreasures));
  ...
  it('renders the treasures table via CharacterTreasuresHelper.render', function() {
    ...
    const html = renderToStaticMarkup(
      CharacterTreasuresHelper.render(treasures, pagination, '#/games/demo/npcs/7/treasures', '#/games/demo/npcs/7')
    );
