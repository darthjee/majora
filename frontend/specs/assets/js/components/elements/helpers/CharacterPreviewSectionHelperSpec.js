  ...
  describe('.render', function() {
    it('renders the title as a heading', function() {
      const html = renderToStaticMarkup(
        CharacterPreviewSectionHelper.render(buildCharacters(2), gameSlug, 'pc', title, seeAllHref)
      );
    ...
    it('renders a card for each character when within the preview limit', function() {
      const html = renderToStaticMarkup(
        CharacterPreviewSectionHelper.render(buildCharacters(3), gameSlug, 'pc', title, seeAllHref)
      );
    ...
    it('slices the characters to the max preview count', function() {
      const html = renderToStaticMarkup(
        CharacterPreviewSectionHelper.render(buildCharacters(8), gameSlug, 'pc', title, seeAllHref)
      );
    ...
    it('renders an empty row when there are no characters', function() {
      const html = renderToStaticMarkup(
        CharacterPreviewSectionHelper.render([], gameSlug, 'pc', title, seeAllHref)
      );
      expect(html).toContain('Player Characters');
      expect(html).toContain(`href="${seeAllHref}"`);
    });
    ...
    it('builds character links for the npc character type', function() {
      const html = renderToStaticMarkup(
        CharacterPreviewSectionHelper.render(buildCharacters(1), gameSlug, 'npc', title, seeAllHref)
      );
