...
describe('AcquireDocumentTab', function() {
  ...
  const renderTab = (props = {}) => {
    ...
    spyOn(AcquireDocumentTabHelper, 'render').and.callFake((state, handlers) => {
      capturedState = state;
      capturedHandlers = handlers;
      return React.createElement('div', null, 'acquire-document-tab');
    });

    renderToStaticMarkup(
      React.createElement(AcquireDocumentTab, {
        show: true,
        character,
  ...
  beforeEach(function() {
    spyOn(AcquireDocumentTabController.prototype, 'loadPage').and.callFake(neverResolves);
  });
  ...
  it('loads the previous page via the controller when onPrev is triggered', function() {
    ...
    expect(AcquireDocumentTabController.prototype.loadPage)
