...
describe('AcquirePossessionTabHelper', function() {
  describe('.render', function() {
    it('passes a null detailPane to TwoColumnLayout when nothing is selected', function() {
      const element = AcquirePossessionTabHelper.render(buildState(), buildHandlers());
    ...
    it('renders a loading message when browsing is loading', function() {
      const element = AcquirePossessionTabHelper.render(
    ...
    it('renders an empty message when there are no possessions to browse', function() {
      const element = AcquirePossessionTabHelper.render(buildState(), buildHandlers());
    ...
    it('renders a list item for each browse possession and wires onSelect', function() {
      const handlers = buildHandlers();
      const item = { id: 9, name: 'Old Tavern' };
    ...
    it('renders the search placeholder', function() {
      ...
      expect(JSON.stringify(element)).toContain('Search possessions...');
    });
  });
});
