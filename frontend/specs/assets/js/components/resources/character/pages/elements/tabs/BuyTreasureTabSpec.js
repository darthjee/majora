...
describe('BuyTreasureTab', function() {
  ...
  const renderTab = (props = {}) => {
    ...
    spyOn(BuyTreasureTabHelper, 'render').and.callFake((state, handlers) => {
      capturedState = state;
      capturedHandlers = handlers;
      return React.createElement('div', null, 'buy-tab');
    });

    renderToStaticMarkup(
      React.createElement(BuyTreasureTab, {
        show: true,
        character,
        ownedTreasures: [],
  ...
  beforeEach(function() {
    spyOn(BuyTreasureTabController.prototype, 'loadPage').and.callFake(neverResolves);
  });
  ...
  it('loads the previous page via the controller when onPrev is triggered', function() {
    ...
    expect(BuyTreasureTabController.prototype.loadPage).toHaveBeenCalledWith(0, character, '', jasmine.any(Function));
  });
  ...
  it('loads the next page via the controller when onNext is triggered', function() {
    ...
    expect(BuyTreasureTabController.prototype.loadPage).toHaveBeenCalledWith(2, character, '', jasmine.any(Function));
  });
