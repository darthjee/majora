...
describe('RemovePossessionTabController', function() {
  describe('#confirmRemove', function() {
    ...
    const selected = { id: 3, game_possession_id: 11, name: 'Old Tavern', photo_path: '/tavern.png' };
    ...
    it('removes the selected owned possession\'s game_possession_id', async function() {
    ...
    it('applies the success outcome: purges the possession cache, clears selection, notifies onSuccess, and reloads',
      async function() {
        ...
        expect(setters.onSuccess).toHaveBeenCalledWith({ gamePossessionId: 11 });
    ...
    it('surfaces the error key, does not purge, and does not reload on a failure', async function() {
      ...
      expect(setters.setSubmitting).toHaveBeenCalledWith(false);
      expect(setters.setActionError).toHaveBeenCalledWith('possession_exchange_modal.generic_error');
