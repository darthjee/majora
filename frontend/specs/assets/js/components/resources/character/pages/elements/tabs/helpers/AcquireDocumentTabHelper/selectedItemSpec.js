...
describe('AcquireDocumentTabHelper', function() {
  describe('.render', function() {
    describe('when a document is selected', function() {
      const selected = { id: 9, name: 'Ancient Map', photo_path: '/map.png', hidden: false };
      ...
      it('renders the action error when present', function() {
        const state = buildState({ selected, actionError: 'document_exchange_modal.already_owned_error' });
        const element = AcquireDocumentTabHelper.render(state, buildHandlers());

        expect(JSON.stringify(element)).toContain('This document is already owned.');
      });
