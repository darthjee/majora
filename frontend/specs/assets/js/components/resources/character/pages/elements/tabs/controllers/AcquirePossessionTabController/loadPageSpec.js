  ...
  describe('#loadPage', function() {
    ...
    it('fetches through RequestStore and applies the resulting items/pagination on success', async function() {
      const ensureSpy = spyOn(RequestStore, 'ensure').and.returnValue(Promise.resolve({
        data: [{ id: 1, name: 'Old Tavern' }], pagination: { page: 2, pages: 4, perPage: 10 },
      }));
      const controller = new AcquirePossessionTabController();
      const setBrowse = jasmine.createSpy('setBrowse');

      await controller.loadPage(2, character, 'tavern', setBrowse);
      ...
      expect(ensureSpy).toHaveBeenCalledWith({
        ...
        query: { page: 2, per_page: 10, name: 'tavern' },
      });
      expect(setBrowse).toHaveBeenCalledWith({
        items: [{ id: 1, name: 'Old Tavern' }], page: 2, pages: 4, loading: false, error: '',
      });
    });
