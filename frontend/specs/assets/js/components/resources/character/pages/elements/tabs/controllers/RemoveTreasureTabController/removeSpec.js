...
describe('RemoveTreasureTabController', function() {
  describe('#remove', function() {
    it('returns ok with the new quantity and money on success', async function() {
      ...
      characterClient.removeTreasure.and.returnValue(Promise.resolve(buildResponse(200, { quantity: 0, money: 600 })));
      const controller = new RemoveTreasureTabController(characterClient);

      const result = await controller.remove('demo', 7, true, 'tok', { treasureId: 9, quantity: 1 });
    ...
    it('uses the npc client when isPc is false', async function() {
      const { characterClient } = buildClients();
      characterClient.removeTreasure.and.returnValue(Promise.resolve(buildResponse(200, { quantity: 0, money: 10 })));
      const controller = new RemoveTreasureTabController(characterClient);

      await controller.remove('demo', 7, false, 'tok', { treasureId: 9, quantity: 1 });
    ...
    it('maps the not enough owned error message to its translation key', async function() {
      ...
      const result = await controller.remove('demo', 7, true, 'tok', { treasureId: 9, quantity: 100 });
