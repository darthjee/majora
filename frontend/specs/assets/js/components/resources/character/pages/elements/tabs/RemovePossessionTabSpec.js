...
describe('RemovePossessionTab', function() {
  ...
  const renderTab = (props = {}) => {
    ...
    spyOn(RemovePossessionTabHelper, 'render').and.callFake((state, handlers) => {
      capturedState = state;
      capturedHandlers = handlers;
      return React.createElement('div', null, 'remove-possession-tab');
    });

    renderToStaticMarkup(
      React.createElement(RemovePossessionTab, {
        show: true,
        character,
  ...
  beforeEach(function() {
    spyOn(RemovePossessionTabController.prototype, 'loadPage').and.callFake(neverResolves);
  });
  ...
  it('loads the previous page via the controller when onPrev is triggered', function() {
    ...
    expect(RemovePossessionTabController.prototype.loadPage)
