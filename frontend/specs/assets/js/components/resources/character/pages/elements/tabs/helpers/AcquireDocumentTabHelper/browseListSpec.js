import AcquireDocumentTabHelper
  from '../../../../../../../../../../../assets/js/components/resources/character/pages/elements/tabs/helpers/AcquireDocumentTabHelper.jsx';
import BrowsePager
...
describe('AcquireDocumentTabHelper', function() {
  describe('.render', function() {
    it('passes a null detailPane to TwoColumnLayout when nothing is selected', function() {
      const element = AcquireDocumentTabHelper.render(buildState(), buildHandlers());
    ...
    it('renders a loading message when browsing is loading', function() {
      const element = AcquireDocumentTabHelper.render(
    ...
    it('renders an empty message when there are no documents to browse', function() {
      const element = AcquireDocumentTabHelper.render(buildState(), buildHandlers());
    ...
    it('renders a browse error message', function() {
      ...
      const element = AcquireDocumentTabHelper.render(state, buildHandlers());

      expect(JSON.stringify(element)).toContain('Unable to complete this action. Please try again.');
    });

    it('renders a list item for each browse document and wires onSelect', function() {
      const handlers = buildHandlers();
      const item = { id: 9, name: 'Ancient Map' };
      const state = buildState({ browse: { items: [item], page: 1, pages: 1, loading: false, error: '' } });
      const element = AcquireDocumentTabHelper.render(state, handlers);
    ...
    it('renders the search placeholder', function() {
      ...
      expect(JSON.stringify(element)).toContain('Search documents...');
    });
  });
});
