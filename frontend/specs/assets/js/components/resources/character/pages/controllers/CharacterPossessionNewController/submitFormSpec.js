...
describe('CharacterPossessionNewController', function() {
  describe('#submitForm', function() {
    ...
    it('prevents default, resets status/errors, and submits the fields payload', async function() {
      const controller = new CharacterPossessionNewController('pcs', Noop.noop, setFieldErrors);
      ...
      try {
        await controller.submitForm(
          ...
          'demo',
          '7',
          { name: 'Old Tavern', description: 'A cozy roadside tavern.', hidden: true },
    ...
    it('redirects to the possessions list on success (no per-possession detail page)', async function() {
      const controller = new CharacterPossessionNewController('pcs', Noop.noop, setFieldErrors);
      const fakeWindow = { location: { hash: '' } };
      globalThis.window = fakeWindow;
      ...
      try {
        await controller.submitForm(
          ...
          'demo',
          '7',
          { name: 'Old Tavern', description: '', hidden: false },
          { setStatus, setFieldErrors },
        );
    ...
    it('sets field errors on a 400 response', async function() {
      ...
      const controller = new CharacterPossessionNewController('npcs', Noop.noop, setFieldErrors);
