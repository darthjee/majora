...
describe('AcquireDocumentTabController', function() {
  describe('#fetchPage', function() {
    it('resolves data and pagination through RequestStore (document.availableCollection)', async function() {
      const pagination = { page: 2, pages: 3, perPage: 5 };
      const ensureSpy = spyOn(RequestStore, 'ensure').and.returnValue(Promise.resolve({
        data: [{ id: 1, name: 'Map' }], pagination,
      }));
      const controller = new AcquireDocumentTabController();

      const result = await controller.fetchPage('demo', 'pcs', 7, { page: 2, perPage: 5, search: 'map' });
      ...
      expect(ensureSpy).toHaveBeenCalledWith({
        ...
        query: { page: 2, per_page: 5, name: 'map' },
      });
      expect(result).toEqual({ data: [{ id: 1, name: 'Map' }], pagination });
    });
