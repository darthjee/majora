...
describe('GameCommonItemNewController', function() {
  describe('#submitForm', function() {
    ...
    it('prevents default, resets status/errors, and submits the fields payload', async function() {
      const controller = new GameCommonItemNewController(Noop.noop, setFieldErrors);
      ...
      try {
        await controller.submitForm(
          ...
          'demo',
          {
            name: 'Healing Potion', description: 'Heals wounds.', price: '50', category: 'potion', hidden: true,
          },
    ...
    it('redirects to the common items list on success (no per-common-item detail page)', async function() {
      const controller = new GameCommonItemNewController(Noop.noop, setFieldErrors);
      const fakeWindow = { location: { hash: '' } };
      globalThis.window = fakeWindow;
      ...
      try {
        await controller.submitForm(
          ...
          'demo',
          {
            name: 'Healing Potion', description: '', price: '50', category: 'potion', hidden: false,
          },
          { setStatus, setFieldErrors },
        );
    ...
    it('sets field errors on a 400 response', async function() {
      ...
      await controller.submitForm(
        ...
        'demo',
        {
          name: '', description: '', price: '0', category: 'other', hidden: false,
        },
        { setStatus, setFieldErrors },
      );
