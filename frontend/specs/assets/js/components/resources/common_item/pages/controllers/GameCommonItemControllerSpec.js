import GameCommonItemController
  from '../../../../../../../../assets/js/components/resources/common_item/pages/controllers/GameCommonItemController.js';
import AccessStore from '../../../../../../../../assets/js/utils/access/store/AccessStore.js';
import RequestStore from '../../../../../../../../assets/js/utils/requests/RequestStore.js';

describe('GameCommonItemController', function() {
  let setCommonItem;
  let setLoading;
  let setError;
  let setCanEdit;
  ...
  beforeEach(function() {
    ...
    client.currentHash.and.returnValue('#/games/demo/common_items/5');
    spyOn(AccessStore, 'ensureGameAccess').and.returnValue(Promise.resolve({}));
    spyOn(AccessStore, 'ensureCommonItemPermissions').and.returnValue(Promise.resolve({ can_edit: false }));
    ensureSpy = spyOn(RequestStore, 'ensure').and.returnValue(
      Promise.resolve({ data: { id: 5, name: 'Healing Potion' } }),
    );
  });

  describe('.getParamsFromHash', function() {
    it('extracts the game slug and common item id', function() {
      expect(GameCommonItemController.getParamsFromHash('#/games/demo/common_items/5')).toEqual({
        game_slug: 'demo', id: '5',
      });
    });

    it('defaults to empty strings for a non-matching hash', function() {
      expect(GameCommonItemController.getParamsFromHash('#/games/demo')).toEqual({
        game_slug: '', id: '',
      });
    });
  });
  ...
  describe('#buildEffect', function() {
    it('fetches the common item through RequestStore', async function() {
      const cleanup = new GameCommonItemController(
        setCommonItem, setLoading, setError, setCanEdit, setCanUploadPhoto, client,
      ).buildEffect()();
      ...
      expect(setCommonItem).toHaveBeenCalledWith({ id: 5, name: 'Healing Potion' });
  ...
  describe('canUploadPhoto', function() {
    ...
    it('calls ensureGameAccess with the game slug independently of the common item fetch', async function() {
      await runController();
  ...
  describe('canEdit', function() {
    ...
    it('calls ensureCommonItemPermissions with the game slug independently of the common item fetch',
      async function() {
        await runController();

        expect(AccessStore.ensureCommonItemPermissions).toHaveBeenCalledWith('demo');
      });

    it('is true when the requester can edit the game', async function() {
      AccessStore.ensureCommonItemPermissions.and.returnValue(Promise.resolve({ can_edit: true }));
    ...
    it('is false when the requester cannot edit the game', async function() {
      AccessStore.ensureCommonItemPermissions.and.returnValue(Promise.resolve({ can_edit: false }));
    ...
    it('fails closed to false when the permissions check rejects', async function() {
      AccessStore.ensureCommonItemPermissions.and.returnValue(Promise.reject(new Error('nope')));
