...
describe('StlModelHelper', function() {
  describe('.render', function() {
    it('renders the STL model name', function() {
      ...
      expect(renderToStaticMarkup(StlModelHelper.render(stlModel))).toContain('Goblin Miniature');
    });

    it('renders a back button to the STL models index', function() {
      const html = renderToStaticMarkup(StlModelHelper.render(buildStlModel()));
      expect(html).toContain('href="#/stl_models"');
    });

    it('renders the photo', function() {
      const html = renderToStaticMarkup(StlModelHelper.render(buildStlModel({ photo_url: 'http://example.com/photo.png' })));
      expect(html).toContain('http://example.com/photo.png');
    });

    it('does not render a links section when there are no links', function() {
      const html = renderToStaticMarkup(StlModelHelper.render(buildStlModel({ links: [] })));
    ...
    it('does not render a sources section when there are no sources', function() {
      const html = renderToStaticMarkup(StlModelHelper.render(buildStlModel({ sources: [] })));
    ...
    it('does not render a tags section when there are no tags', function() {
      const html = renderToStaticMarkup(StlModelHelper.render(buildStlModel({ tags: [] })));
