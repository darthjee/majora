...
describe('ItemPhoto', function() {
  ...
  describe('.Edit', function() {
    ...
    it('dims the photo when hidden is true', function() {
      expect(ItemPhoto.Edit(buildProps({ hidden: true })).props.photoState.dimmed).toBe(true);
    });

    it('does not dim the photo when hidden is false', function() {
      expect(ItemPhoto.Edit(buildProps({ hidden: false })).props.photoState.dimmed).toBe(false);
    });
  ...
  describe('.New', function() {
    ...
    it('dims the photo when hidden is true', function() {
      expect(ItemPhoto.New(buildProps({ hidden: true })).props.photoState.dimmed).toBe(true);
    });

    it('does not dim the photo when hidden is false', function() {
      expect(ItemPhoto.New(buildProps({ hidden: false })).props.photoState.dimmed).toBe(false);
    });
