...
describe('ItemNameField', function() {
  ...
  it('scopes the id to edit mode', function() {
    const element = ItemNameField(buildProps({ mode: 'edit' }));

    expect(element.props.id).toBe('item-edit-name');
  });

  it('passes field errors through', function() {
    const element = ItemNameField(buildProps({ fieldErrors: { name: ['is too short'] } }));
  ...
  it('wires onChange to handlers.onNameChange', function() {
    ...
    const element = ItemNameField(buildProps({ handlers }));
