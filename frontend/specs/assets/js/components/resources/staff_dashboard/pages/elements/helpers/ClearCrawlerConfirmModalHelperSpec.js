...
describe('ClearCrawlerConfirmModalHelper', function() {
  const buildHandlers = () => ({
  ...
  describe('.render', function() {
    it('renders the title and body from the clear_crawler_confirm_modal i18n namespace', function() {
      const element = ClearCrawlerConfirmModalHelper.render(true, buildHandlers());
      const title = findElement(element, (child) => child.type === Modal.Title);

      expect(title.props.children).toBe(Translator.t('clear_crawler_confirm_modal.title'));
      expect(JSON.stringify(element)).toContain(Translator.t('clear_crawler_confirm_modal.body'));
    });
    ...
    it('uses the danger variant for the confirm button', function() {
      ...
      expect(confirmButton.props.children).toBe(Translator.t('clear_crawler_confirm_modal.confirm'));
    });

    it('wires the cancel and confirm handlers', function() {
      const handlers = buildHandlers();
      const element = ClearCrawlerConfirmModalHelper.render(true, handlers);
    ...
    it('respects the show flag', function() {
      const element = ClearCrawlerConfirmModalHelper.render(false, buildHandlers());
      const modal = findElement(element, (child) => child.type === Modal);
