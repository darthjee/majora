...
describe('RecruitModalHelper', function() {
  ...
  describe('.render', function() {
    it('renders the modal title', function() {
      const element = RecruitModalHelper.render(true, buildState(), buildHandlers());
      const title = findElement(element, (node) => node.type === Modal.Title);

      expect(title.props.children).toBe('Recruit');
    });

    it('renders the pc/npc tabs', function() {
      const element = RecruitModalHelper.render(true, buildState({ activeTab: 'npcs' }), buildHandlers());
    ...
    it('binds the search input value and onChange handler', function() {
      ...
      const element = RecruitModalHelper.render(true, buildState({ search: 'ar' }), handlers);
    ...
    it('renders the loading message when browsing', function() {
      const element = RecruitModalHelper.render(
    ...
    it('renders the load error message when browsing fails', function() {
      const element = RecruitModalHelper.render(
        ...
        buildState({
          browse: {
            items: [], page: 1, pages: 1, loading: false, error: 'recruit_modal.load_error',
          },
        }),
        buildHandlers(),
      );
    ...
    it('renders a browse-list entry per character and invokes onSelectCharacter when clicked', function() {
      const handlers = buildHandlers();
      const element = RecruitModalHelper.render(true, buildState(), handlers);
    ...
    it('passes a null detailPane to TwoColumnLayout when the receiving list is empty', function() {
      const element = RecruitModalHelper.render(true, buildState({ receiving: [] }), buildHandlers());
      const layout = findElement(element, (node) => node.type === TwoColumnLayout);

      expect(layout.props.detailPane).toBeNull();
    });

    it('renders the receiving list through RecruitReceivingRowHelper when non-empty', function() {
      const row = {
        character: { id: 1, name: 'Aria' }, kind: 'pcs', enlisted: false, result: null,
      };
      spyOn(RecruitReceivingRowHelper, 'render').and.returnValue(null);

      RecruitModalHelper.render(true, buildState({ receiving: [row] }), buildHandlers());

      expect(RecruitReceivingRowHelper.render).toHaveBeenCalledWith(row, jasmine.any(Object));
    });
    ...
    it('passes a non-null detailPane to TwoColumnLayout when the receiving list is non-empty', function() {
      ...
      const element = RecruitModalHelper.render(true, buildState({ receiving: [row] }), buildHandlers());
      const layout = findElement(element, (node) => node.type === TwoColumnLayout);
    ...
    it('disables clear/cancel/submit while submitting', function() {
      const element = RecruitModalHelper.render(true, buildState({ submitting: true }), buildHandlers());
    ...
    it('renders the modal close button when not submitting', function() {
      const element = RecruitModalHelper.render(true, buildState({ submitting: false }), buildHandlers());
      const header = findElement(element, (node) => node.type === Modal.Header);
    ...
    it('invokes onSubmit when Submit is clicked', function() {
      ...
      const element = RecruitModalHelper.render(true, buildState({ receiving: [row] }), handlers);
      const submitButton = findElement(element, (node) => node.type === 'button' && node.props.children === 'Submit');
    ...
    it('does not call onClose when hiding the modal while submitting', function() {
      const handlers = buildHandlers();
      const element = RecruitModalHelper.render(true, buildState({ submitting: true }), handlers);
    ...
    it('calls onClose when hiding the modal while not submitting', function() {
      const handlers = buildHandlers();
      const element = RecruitModalHelper.render(true, buildState({ submitting: false }), handlers);
