...
describe('CollectionNewModalHelper', function() {
  const buildHandlers = () => ({
  ...
  describe('.render', function() {
    it('renders a Modal with the given show flag', function() {
      const element = CollectionNewModalHelper.render(true, buildState(), buildHandlers());
    ...
    it('wires onHide to the onClose handler', function() {
      ...
      const element = CollectionNewModalHelper.render(true, buildState(), handlers);
    ...
    it('renders the modal title', function() {
      ...
      expect(findText(title)).toBe('New Collection');
    });
    ...
    it('renders the name field seeded with the current name value', function() {
      const element = CollectionNewModalHelper.render(true, buildState(), buildHandlers());
      const input = findElement(element, (child) => child.type === 'input' && child.props.id === 'collection-new-name');

      expect(input.props.value).toBe('Goblin Pack');
    });
    ...
    it('renders a full-width submit button in the footer', function() {
      ...
      expect(findText(button)).toBe('Create Collection');
    });

    it('disables the submit button while submitting', function() {
      const element = CollectionNewModalHelper.render(true, buildState({ status: 'submitting' }), buildHandlers());
      const footer = findElement(element, (child) => child.type === Modal.Footer);
      const button = findElement(footer, (child) => child.props?.type === 'submit');
    ...
    it('renders per-field errors when present', function() {
      const element = CollectionNewModalHelper.render(
    ...
    it('renders a general error alert when status is error', function() {
      const element = CollectionNewModalHelper.render(true, buildState({ status: 'error' }), buildHandlers());
      const errorAlert = findElement(element, (child) => child.props?.className === 'alert alert-danger');

      expect(findText(errorAlert)).toBe('Failed to create collection. Please try again.');
    });
    ...
    it('renders the photo-upload-failed warning with retry/skip actions when status is photo-upload-failed', function() {
      const handlers = buildHandlers();
      const element = CollectionNewModalHelper.render(true, buildState({ status: 'photo-upload-failed' }), handlers);
