  ...
  describe('#submitForm photo upload saga', function() {
    ...
    beforeEach(function() {
      ...
      spyOn(RequestStore, 'mutate').and.returnValue(Promise.resolve({
        status: 201,
        json: () => Promise.resolve({ id: 7, name: 'Goblin Pack', url: '' }),
      }));
    ...
    const buildFormValues = () => ({ name: 'Goblin Pack', url: '', photoFile });

    it('uploads the photo and calls onSuccess when the collection is created and the upload succeeds', async function() {
      ...
      const controller = new CollectionNewController(setError, setFieldErrors, uploadClient);
