...
describe('AccessStorePermissions', function() {
  ...
  describe('#ensureFaction', function() {
    it('fetches, caches, and returns the faction permissions payload for an anonymous (default) access',
      async function() {
        const gameClient = jasmine.createSpyObj('gameClient', ['fetchGameAccess', 'fetchFactionPermissions']);
        gameClient.fetchGameAccess.and.returnValue(Promise.resolve(fakeResponse({})));
        gameClient.fetchFactionPermissions.and.returnValue(Promise.resolve(fakeResponse({ can_edit: true })));

        const result = await AccessStorePermissions.ensureFaction(cache, gameClient, 'demo');

        expect(result).toEqual({ can_edit: true });
        expect(gameClient.fetchFactionPermissions).toHaveBeenCalledWith('demo', null, jasmine.anything(), []);
        expect(gameClient.fetchFactionPermissions).toHaveBeenCalledTimes(1);
      });
    ...
    it('derives the real role set from the already-cached game access entry', async function() {
      ...
      const gameClient = jasmine.createSpyObj('gameClient', ['fetchFactionPermissions']);
      gameClient.fetchFactionPermissions.and.returnValue(Promise.resolve(fakeResponse({ can_edit: true })));

      await AccessStorePermissions.ensureFaction(cache, gameClient, 'demo');

      expect(gameClient.fetchFactionPermissions)
        .toHaveBeenCalledWith('demo', null, jasmine.anything(), ['dm', 'logged']);
    });

    it('fires exactly one permissions fetch when access is already resolved/cached before ensureFaction is called',
      async function() {
        await seedGameAccess(cache, { is_dm: true, is_logged: true });
    ...
    it('issues a corrected fetch when access resolves after the optimistic fetch with a different role set',
      async function() {
        ...
        gameClient.fetchFactionPermissions.and.returnValues(
          Promise.resolve(fakeResponse({ can_edit: false })),
          Promise.resolve(fakeResponse({ can_edit: true })),
        );

        const resultPromise = AccessStorePermissions.ensureFaction(cache, gameClient, 'demo');
        resolveAccess(fakeResponse({ is_dm: true, is_logged: true }));
        const result = await resultPromise;

        expect(result).toEqual({ can_edit: true });
        expect(gameClient.fetchFactionPermissions).toHaveBeenCalledTimes(2);
        expect(gameClient.fetchFactionPermissions.calls.argsFor(0))
          .toEqual(['demo', null, jasmine.anything(), []]);
        expect(gameClient.fetchFactionPermissions.calls.argsFor(1))
    ...
    it('resolves to the fail-closed default when the response is not ok', async function() {
      const gameClient = jasmine.createSpyObj('gameClient', ['fetchGameAccess', 'fetchFactionPermissions']);
      gameClient.fetchGameAccess.and.returnValue(Promise.resolve(fakeResponse({})));
      gameClient.fetchFactionPermissions.and.returnValue(Promise.resolve(fakeResponse(null, false)));
