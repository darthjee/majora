...
describe('AccessStorePermissions', function() {
  ...
  describe('#ensureItem', function() {
    it('fetches, caches, and returns the item permissions payload for an anonymous (default) access',
      async function() {
        const gameClient = jasmine.createSpyObj('gameClient', ['fetchGameAccess', 'fetchItemPermissions']);
        gameClient.fetchGameAccess.and.returnValue(Promise.resolve(fakeResponse({})));
        gameClient.fetchItemPermissions.and.returnValue(Promise.resolve(fakeResponse({ can_edit: true })));

        const result = await AccessStorePermissions.ensureItem(cache, gameClient, 'demo');

        expect(result).toEqual({ can_edit: true });
        expect(gameClient.fetchItemPermissions).toHaveBeenCalledWith('demo', null, jasmine.anything(), []);
        expect(gameClient.fetchItemPermissions).toHaveBeenCalledTimes(1);
      });
    ...
    it('derives the real role set from the already-cached game access entry', async function() {
      ...
      const gameClient = jasmine.createSpyObj('gameClient', ['fetchItemPermissions']);
      gameClient.fetchItemPermissions.and.returnValue(Promise.resolve(fakeResponse({ can_edit: true })));

      await AccessStorePermissions.ensureItem(cache, gameClient, 'demo');

      expect(gameClient.fetchItemPermissions)
        .toHaveBeenCalledWith('demo', null, jasmine.anything(), ['dm', 'logged']);
    });

    it('fires exactly one permissions fetch when access is already resolved/cached before ensureItem is called',
      async function() {
        await seedGameAccess(cache, { is_dm: true, is_logged: true });
    ...
    it('issues a corrected fetch when access resolves after the optimistic fetch with a different role set',
      async function() {
        ...
        gameClient.fetchItemPermissions.and.returnValues(
          Promise.resolve(fakeResponse({ can_edit: false })),
          Promise.resolve(fakeResponse({ can_edit: true })),
        );

        const resultPromise = AccessStorePermissions.ensureItem(cache, gameClient, 'demo');
        resolveAccess(fakeResponse({ is_dm: true, is_logged: true }));
        const result = await resultPromise;

        expect(result).toEqual({ can_edit: true });
        expect(gameClient.fetchItemPermissions).toHaveBeenCalledTimes(2);
        expect(gameClient.fetchItemPermissions.calls.argsFor(0))
          .toEqual(['demo', null, jasmine.anything(), []]);
        expect(gameClient.fetchItemPermissions.calls.argsFor(1))
    ...
    it('resolves to the fail-closed default when the response is not ok', async function() {
      const gameClient = jasmine.createSpyObj('gameClient', ['fetchGameAccess', 'fetchItemPermissions']);
      gameClient.fetchGameAccess.and.returnValue(Promise.resolve(fakeResponse({})));
      gameClient.fetchItemPermissions.and.returnValue(Promise.resolve(fakeResponse(null, false)));
