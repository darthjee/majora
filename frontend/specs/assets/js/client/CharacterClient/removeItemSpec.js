...
describe('CharacterClient', function() {
  ...
  describe('#removeItem', function() {
    it('sends a POST request with the fields and auth token for a PC', async function() {
      ...
      await client.removeItem('pcs', 'demo', '2', 'abc123', { game_item_id: 9 });

      expect(fetchSpy).toHaveBeenCalledWith('/games/demo/pcs/2/items/remove.json', jasmine.objectContaining({
        method: 'POST',
        headers: {
    ...
    it('sends a POST request with the fields and auth token for an NPC', async function() {
      ...
      await client.removeItem('npcs', 'demo', '2', 'abc123', { game_item_id: 9 });

      expect(fetchSpy).toHaveBeenCalledWith('/games/demo/npcs/2/items/remove.json', jasmine.objectContaining({
        method: 'POST',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
