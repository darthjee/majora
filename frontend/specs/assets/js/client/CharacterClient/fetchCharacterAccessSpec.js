...
describe('CharacterClient', function() {
  ...
  describe('#fetchCharacterAccess', function() {
    it('requests the access endpoint with the auth token when present for a PC', async function() {
      const client = new CharacterClient();

      await client.fetchCharacterAccess('pcs', 'demo', '2', 'abc123');
    ...
    it('omits the Authorization header when there is no token for a PC', async function() {
      ...
      await client.fetchCharacterAccess('pcs', 'demo', '2', null);
    ...
    it('requests the access endpoint with the auth token when present for an NPC', async function() {
      const client = new CharacterClient();

      await client.fetchCharacterAccess('npcs', 'demo', '2', 'abc123');
    ...
    it('omits the Authorization header when there is no token for an NPC', async function() {
      ...
      await client.fetchCharacterAccess('npcs', 'demo', '2', null);
