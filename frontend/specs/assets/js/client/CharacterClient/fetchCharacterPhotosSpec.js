...
describe('CharacterClient', function() {
  ...
  describe('#fetchCharacterPhotos', function() {
    ...
    it('defaults per_page to 6 when not provided', async function() {
      ...
      expect(fetchSpy).toHaveBeenCalledWith('/games/demo/pcs/2/photos.json?per_page=6', {
        method: 'GET',
        headers: {
    ...
    it('requests a custom per_page value when provided', async function() {
      ...
      expect(fetchSpy).toHaveBeenCalledWith('/games/demo/npcs/2/photos.json?per_page=10', {
        method: 'GET',
        headers: {
          Accept: 'application/json',
          Authorization: 'Token abc123',
